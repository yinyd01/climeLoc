...
/*! @brief	kernel functions for PSFfit_SINPA
	...
	@param[in]	splineszx: 			int, the size of the splines cube (lateral-axis)
	...
	@param[in]	g_coeff_PSF: 		(nchannels * splineszx * splineszx * splineszz* 64) spline coefficient for the 3D-model PSF
	@param[in]	opt:				int, optimization method. 0 for MLE and 1 for LSQ
	@param[out]	g_xvec: 			(NFits * vnum) see definitions.h
	@param[out]	g_CRLB:				(NFits * vnum) the CRLB corresponding to each of the variables in xvec, respectively
	@param[out]	g_Loss:				(NFits) Loss value for the optimization of a PSF fitting.
	@param		g_push_flag: 		(NFits) flag for whether iteration should proceed.
	@param		g_maxJump: 			(NFits * vnum), maxJump control vector for each of the parameter for xvec.
	@param		g_grad: 			(NFits * vnum), gradient vector of w.r.t the xvec.
	@param		g_Hessian: 			(NFits * vnum * vnum), Hessian matrix w.r.t the xvec.
	@param		g_pvec: 			(NFits * vnum), increment of xvec for a single iteration.
	@param		g_lambda: 			(NFits), lambda value for Levenberg-Marquardt optimization.	
*/
...
*/
__global__ void Init(int NFits, int boxsz, int splineszx, int splineszz, float* g_data, float* g_var, float* coeff_PSF,
	float* g_xvec, float* g_maxJump, float* g_Loss, float* g_grad, float* g_Hessian, float* g_pvec, float* g_lambda, char* g_push_flag, int opt)
{
	...
	if (Idx < NFits) {
		...
		init_xvec(boxsz, splineszz, dataim, xvec);
		init_maxJump(boxsz, splineszz, xvec, maxJump);
		get_lgH(boxsz, splineszx, splineszz, dataim, varim, coeff_PSF, xvec, &Loss, grad, Hessian, opt);
...
*/
__global__ void LMupdate(int NFits, char* g_push_flag, int boxsz, int splineszx, int splineszz, float* g_data, float* g_var, float* coeff_PSF, 
	float* g_maxJump, float* g_lambda, float* g_xvec, float* g_grad, float* g_Hessian, float* g_pvec, float* g_Loss, int opt)
{
	...
	if (push_flag) {
		...
		if ( decomp_flag == 1) {
			...
			boxConstrain(boxsz, splineszz, xvec);
...
/*!	@brief	<<<ceil(NFits/BLCKSZ), BLCKSZ = 64>>>
			calculate the CRLB	
*/
__global__ void getCRLB(int NFits, int boxsz, int splineszx, int splineszz, float* g_var, float* coeff_PSF, 
	float* g_xvec, float* g_Loss, float* g_Hessian, float* g_CRLB, int opt)
{
	...
	if (Idx < NFits){
		...
		// construct the Fisher Information Matrix (FIM), use svdV and pinvF to store data and model
		if (opt == 0) {
			varim = (g_var) ? g_var + Idx * boxsz * boxsz : nullptr;
			get_FIM(boxsz, splineszx, splineszz, varim, coeff_PSF, xvec, FIM);
		}
		else
			memcpy(FIM, g_Hessian + Idx * vnum, vnum * sizeof(float));

...
CDLL_EXPORT void cspline3d(int NFits, int boxsz, int splineszx, int splineszz, float* h_data, float* h_var, float* h_coeff_PSF,
    float* h_xvec, float* h_CRLB, float* h_Loss, int opt, int MaxIters)
{
    ...
    Init<<<grids, blcks>>>(NFits, boxsz, splineszx, splineszz, d_data, d_var, d_coeff_PSF,
        d_xvec, d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_push_flag, opt);

    for (int iter = 0; iter < MaxIters; iter++)
        LMupdate<<<grids, blcks>>>(NFits, d_push_flag, boxsz, splineszx, splineszz, d_data, d_var, d_coeff_PSF, 
            d_maxJump, d_lambda, d_xvec, d_grad, d_Hessian, d_pvec, d_Loss, opt);
    
    getCRLB<<<grids, blcks>>>(NFits, boxsz, splineszx, splineszz, d_var, d_coeff_PSF, d_xvec, d_Loss, d_Hessian, d_CRLB, opt);

...
*/
__global__ void Init(int NFits, int boxsz, int splinesz, float* g_data, float* g_var, float* coeff_PSF, 
	float* g_xvec, float* g_maxJump, float* g_Loss, float* g_grad, float* g_Hessian, float* g_pvec, float* g_lambda, char* g_push_flag, int opt)
{
	...
	float Loss = 0.0f;
	float xvec[VNUM] = {0.0f}, maxJump[VNUM] = {0.0f}, grad[VNUM] = {0.0f}, Hessian[VNUM * VNUM] = {0.0f};
	...
	if (Idx < NFits) {
		...
		init_xvec(boxsz, dataim, xvec);
		init_maxJump(boxsz, xvec, maxJump);
		get_lgH(boxsz, splinesz, dataim, varim, coeff_PSF, xvec, &Loss, grad, Hessian, opt);
...
*/
__global__ void LMupdate(int NFits, char* g_push_flag, int boxsz, int splinesz, float* g_data, float* g_var, float* coeff_PSF,
...
/*!	@brief	<<<ceil(NFits/BLCKSZ), BLCKSZ = 64>>>
			calculate the CRLB	
*/
__global__ void getCRLB(int NFits, int boxsz, int splinesz, float* g_var, float* coeff_PSF, 
	float* g_xvec, float* g_Loss, float* g_Hessian, float* g_CRLB, int opt)
{
	...
	if (Idx < NFits) {
		...
		if (opt == 0) {
			varim = (g_var) ? g_var + Idx * boxsz * boxsz : nullptr;
			get_FIM(boxsz, splinesz, varim, coeff_PSF, xvec, FIM);
		}
		else

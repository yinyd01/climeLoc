...
#ifndef KERNELS_3_CUH
#define KERNELS_3_CUH
...
*/
__global__ void PushnPull_3(int NFits, int nchannels, int boxsz, int zrange, float* g_data, float* g_astigs, float* g_xvec)
{
	const int nnum = 3, vnum0 = NDIM + nchannels;
...
*/
__global__ void Init_3_1ch(int NFits, int boxsz, int zrange, float* g_data, float* g_var, float* astigs, float* g_xvec, 
	float* g_maxJump, float* g_Loss, float* g_grad, float* g_Hessian, float* g_pvec, float* g_lambda, char* g_iter_push)
{
	const int nnum = 3, nchannels = 1, vnum0 = NDIM + nchannels;	
	const int vnum = nnum * vnum0 + nchannels;
	unsigned int Idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void Init_3_2ch(int NFits, int boxsz, int zrange, float* g_data, float* g_var, float* astigs, 
	int warpdeg, int* g_lu, float* coeff_R2T, float* g_xvec,  
	float* g_maxJump, float* g_Loss, float* g_grad, float* g_Hessian, float* g_pvec, float* g_lambda, char* g_iter_push)
{
	const int nnum = 3, nchannels = 2, vnum0 = NDIM + nchannels;
	const int vnum = nnum * vnum0 + nchannels;
	unsigned int Idx = blockIdx.x * blockDim.x + threadIdx.x;
	float *dataim = nullptr, *varim = nullptr;
	float *xvec = nullptr, *maxJump = nullptr;
...
__global__ void LMupdate_3_1ch(int NFits, char* g_iter_push, int boxsz, int zrange, float* g_data, float* g_var, float* astigs,
...
__global__ void LMupdate_3_2ch(int NFits, char* g_iter_push, int boxsz, int zrange, float* g_data, float* g_var, float* astigs, 
...
__global__ void getCRLB_3_1ch(int NFits, int boxsz, float* g_var, float* astigs, float* g_xvec, float* g_CRLB)
{
	const int nnum = 3, nchannels = 1, vnum0 = NDIM + nchannels;
	const int vnum = nnum * vnum0 + nchannels;
	unsigned int Idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void getCRLB_3_2ch(int NFits, int boxsz, float* g_var, float* astigs, 

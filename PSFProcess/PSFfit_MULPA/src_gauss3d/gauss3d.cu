...
/*! @param[in]	NFits: 				int, number of PSF squares
	...
	@param[in]	zrange:				int, range (axial-axis) for fit
    ...
    @param[in]	h_astigs:			(nchannels * 9) float, [PSFsigmax0, PSFsigmay0, shiftx, shifty, dof, Ax, Bx, Ay, By] the astigmatic parameters for each channel
...
CDLL_EXPORT void gauss3d_1ch(int NFits, int nPSF, int boxsz, int zrange, float* h_data, float* h_var, float* h_astigs, 
    float* h_xvec, float* h_CRLB, float* h_Loss, int opt, int MaxIters)
{
    ...
    float *d_data = nullptr, *d_var = nullptr, *d_astigs = nullptr;
    ...
    cudasafe(cudaMalloc((void**)&d_astigs, nchannels * 9 * sizeof(float)), "malloc d_astigs", __LINE__);
    ...
    cudasafe(cudaMemcpy(d_astigs, h_astigs, nchannels * 9 * sizeof(float), cudaMemcpyHostToDevice), "transfer h_astigs to device", __LINE__);
    ...
    if (nnum == 1) {   
        PreInit<<<grids, blcks>>>(NFits, nchannels, boxsz, zrange, d_data, d_xvec);
        Init_1_1ch<<<grids, blcks>>>(NFits, boxsz, zrange, d_data, d_var, d_astigs, d_xvec,  
            d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_iter_push);
        for (unsigned int iter = 0; iter < MaxIters; iter++)
            LMupdate_1_1ch<<<grids, blcks>>>(NFits, d_iter_push, boxsz, zrange, d_data, d_var, d_astigs,
                d_xvec, d_Loss, d_maxJump, d_lambda, d_grad, d_Hessian, d_pvec);
        getCRLB_1_1ch<<<grids, blcks>>>(NFits, boxsz, d_var, d_astigs, d_xvec, d_CRLB);
    }
    else if (nnum == 2) {   
        PushnPull_2<<<grids, blcks>>>(NFits, nchannels, boxsz, zrange, d_data, d_astigs, d_xvec);
        Init_2_1ch<<<grids, blcks>>>(NFits, boxsz, zrange, d_data, d_var, d_astigs, d_xvec,  
            d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_iter_push);
        for (unsigned int iter = 0; iter < MaxIters; iter++)
            LMupdate_2_1ch<<<grids, blcks>>>(NFits, d_iter_push, boxsz, zrange, d_data, d_var, d_astigs,
                d_xvec, d_Loss, d_maxJump, d_lambda, d_grad, d_Hessian, d_pvec);
        getCRLB_2_1ch<<<grids, blcks>>>(NFits, boxsz, d_var, d_astigs, d_xvec, d_CRLB);
    }
    else if (nnum == 3) {   
        PushnPull_3<<<grids, blcks>>>(NFits, nchannels, boxsz, zrange, d_data, d_astigs, d_xvec);
        Init_3_1ch<<<grids, blcks>>>(NFits, boxsz, zrange, d_data, d_var, d_astigs, d_xvec,  
            d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_iter_push);
        for (unsigned int iter = 0; iter < MaxIters; iter++)
            LMupdate_3_1ch<<<grids, blcks>>>(NFits, d_iter_push, boxsz, zrange, d_data, d_var, d_astigs,
                d_xvec, d_Loss, d_maxJump, d_lambda, d_grad, d_Hessian, d_pvec);
        getCRLB_3_1ch<<<grids, blcks>>>(NFits, boxsz, d_var, d_astigs, d_xvec, d_CRLB);
    }
    else if (nnum == 4) {   
        PushnPull_4<<<grids, blcks>>>(NFits, nchannels, boxsz, zrange, d_data, d_astigs, d_xvec);
        Init_4_1ch<<<grids, blcks>>>(NFits, boxsz, zrange, d_data, d_var, d_astigs, d_xvec,  
            d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_iter_push);
        for (unsigned int iter = 0; iter < MaxIters; iter++)
            LMupdate_4_1ch<<<grids, blcks>>>(NFits, d_iter_push, boxsz, zrange, d_data, d_var, d_astigs,
                d_xvec, d_Loss, d_maxJump, d_lambda, d_grad, d_Hessian, d_pvec);
        getCRLB_4_1ch<<<grids, blcks>>>(NFits, boxsz, d_var, d_astigs, d_xvec, d_CRLB);
    }
    else if (nnum == 5) {   
        PushnPull_5<<<grids, blcks>>>(NFits, nchannels, boxsz, zrange, d_data, d_astigs, d_xvec);
        Init_5_1ch<<<grids, blcks>>>(NFits, boxsz, zrange, d_data, d_var, d_astigs, d_xvec,  
            d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_iter_push);
        for (unsigned int iter = 0; iter < MaxIters; iter++)
            LMupdate_5_1ch<<<grids, blcks>>>(NFits, d_iter_push, boxsz, zrange, d_data, d_var, d_astigs,
                d_xvec, d_Loss, d_maxJump, d_lambda, d_grad, d_Hessian, d_pvec);
        getCRLB_5_1ch<<<grids, blcks>>>(NFits, boxsz, d_var, d_astigs, d_xvec, d_CRLB);
    }
    else if (nnum == 6) {   
        PushnPull_6<<<grids, blcks>>>(NFits, nchannels, boxsz, zrange, d_data, d_astigs, d_xvec);
        Init_6_1ch<<<grids, blcks>>>(NFits, boxsz, zrange, d_data, d_var, d_astigs, d_xvec,  
            d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_iter_push);
        for (unsigned int iter = 0; iter < MaxIters; iter++)
            LMupdate_6_1ch<<<grids, blcks>>>(NFits, d_iter_push, boxsz, zrange, d_data, d_var, d_astigs,
                d_xvec, d_Loss, d_maxJump, d_lambda, d_grad, d_Hessian, d_pvec);
        getCRLB_6_1ch<<<grids, blcks>>>(NFits, boxsz, d_var, d_astigs, d_xvec, d_CRLB);
    }
    else if (nnum == 7) {   
        PushnPull_7<<<grids, blcks>>>(NFits, nchannels, boxsz, zrange, d_data, d_astigs, d_xvec);
        Init_7_1ch<<<grids, blcks>>>(NFits, boxsz, zrange, d_data, d_var, d_astigs, d_xvec,  
            d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_iter_push);
        for (unsigned int iter = 0; iter < MaxIters; iter++)
            LMupdate_7_1ch<<<grids, blcks>>>(NFits, d_iter_push, boxsz, zrange, d_data, d_var, d_astigs,
                d_xvec, d_Loss, d_maxJump, d_lambda, d_grad, d_Hessian, d_pvec);
        getCRLB_7_1ch<<<grids, blcks>>>(NFits, boxsz, d_var, d_astigs, d_xvec, d_CRLB);
    }
    ...
    cudasafe(cudaFree(d_astigs), "free d_astigs on device", __LINE__);
...
CDLL_EXPORT void gauss3d_2ch(int NFits, int nPSF, int boxsz, int zrange, float* h_data, float* h_var, float* h_astigs, 
...
{    
    ...
    float *d_data = nullptr, *d_var = nullptr, *d_astigs = nullptr, *d_coeff_R2T = nullptr;
    int *d_lu = nullptr;
    ...
    if (nnum == 1) {   
        PreInit<<<grids, blcks>>>(NFits, nchannels, boxsz, zrange, d_data, d_xvec);
        Init_1_2ch<<<grids, blcks>>>(NFits, boxsz, zrange, d_data, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, d_xvec,  
            d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_iter_push);
        for (unsigned int iter = 0; iter < MaxIters; iter++)
            LMupdate_1_2ch<<<grids, blcks>>>(NFits, d_iter_push, boxsz, zrange, d_data, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, 
                d_xvec, d_Loss, d_maxJump, d_lambda, d_grad, d_Hessian, d_pvec);
        getCRLB_1_2ch<<<grids, blcks>>>(NFits, boxsz, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, d_xvec, d_CRLB);
    }
    else if (nnum == 2) {   
        PushnPull_2<<<grids, blcks>>>(NFits, nchannels, boxsz, zrange, d_data, d_astigs, d_xvec);
        Init_2_2ch<<<grids, blcks>>>(NFits, boxsz, zrange, d_data, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, d_xvec,  
            d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_iter_push);
        for (unsigned int iter = 0; iter < MaxIters; iter++)
            LMupdate_2_2ch<<<grids, blcks>>>(NFits, d_iter_push, boxsz, zrange, d_data, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, 
                d_xvec, d_Loss, d_maxJump, d_lambda, d_grad, d_Hessian, d_pvec);
        getCRLB_2_2ch<<<grids, blcks>>>(NFits, boxsz, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, d_xvec, d_CRLB);
    }
    else if (nnum == 3) {   
        PushnPull_3<<<grids, blcks>>>(NFits, nchannels, boxsz, zrange, d_data, d_astigs, d_xvec);
        Init_3_2ch<<<grids, blcks>>>(NFits, boxsz, zrange, d_data, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, d_xvec,  
            d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_iter_push);
        for (unsigned int iter = 0; iter < MaxIters; iter++)
            LMupdate_3_2ch<<<grids, blcks>>>(NFits, d_iter_push, boxsz, zrange, d_data, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, 
                d_xvec, d_Loss, d_maxJump, d_lambda, d_grad, d_Hessian, d_pvec);
        getCRLB_3_2ch<<<grids, blcks>>>(NFits, boxsz, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, d_xvec, d_CRLB);
    }
    else if (nnum == 4) {   
        PushnPull_4<<<grids, blcks>>>(NFits, nchannels, boxsz, zrange, d_data, d_astigs, d_xvec);
        Init_4_2ch<<<grids, blcks>>>(NFits, boxsz, zrange, d_data, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, d_xvec,  
            d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_iter_push);
        for (unsigned int iter = 0; iter < MaxIters; iter++)
            LMupdate_4_2ch<<<grids, blcks>>>(NFits, d_iter_push, boxsz, zrange, d_data, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, 
                d_xvec, d_Loss, d_maxJump, d_lambda, d_grad, d_Hessian, d_pvec);
        getCRLB_4_2ch<<<grids, blcks>>>(NFits, boxsz, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, d_xvec, d_CRLB);
    }
    else if (nnum == 5) {   
        PushnPull_5<<<grids, blcks>>>(NFits, nchannels, boxsz, zrange, d_data, d_astigs, d_xvec);
        Init_5_2ch<<<grids, blcks>>>(NFits, boxsz, zrange, d_data, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, d_xvec,  
            d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_iter_push);
        for (unsigned int iter = 0; iter < MaxIters; iter++)
            LMupdate_5_2ch<<<grids, blcks>>>(NFits, d_iter_push, boxsz, zrange, d_data, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, 
                d_xvec, d_Loss, d_maxJump, d_lambda, d_grad, d_Hessian, d_pvec);
        getCRLB_5_2ch<<<grids, blcks>>>(NFits, boxsz, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, d_xvec, d_CRLB);
    }
    else if (nnum == 6) {   
        PushnPull_6<<<grids, blcks>>>(NFits, nchannels, boxsz, zrange, d_data, d_astigs, d_xvec);
        Init_6_2ch<<<grids, blcks>>>(NFits, boxsz, zrange, d_data, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, d_xvec,  
            d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_iter_push);
        for (unsigned int iter = 0; iter < MaxIters; iter++)
            LMupdate_6_2ch<<<grids, blcks>>>(NFits, d_iter_push, boxsz, zrange, d_data, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, 
                d_xvec, d_Loss, d_maxJump, d_lambda, d_grad, d_Hessian, d_pvec);
        getCRLB_6_2ch<<<grids, blcks>>>(NFits, boxsz, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, d_xvec, d_CRLB);
    }
    else if (nnum == 7) {   
        PushnPull_7<<<grids, blcks>>>(NFits, nchannels, boxsz, zrange, d_data, d_astigs, d_xvec);
        Init_7_2ch<<<grids, blcks>>>(NFits, boxsz, zrange, d_data, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, d_xvec,  
            d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_iter_push);
        for (unsigned int iter = 0; iter < MaxIters; iter++)
            LMupdate_7_2ch<<<grids, blcks>>>(NFits, d_iter_push, boxsz, zrange, d_data, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, 
                d_xvec, d_Loss, d_maxJump, d_lambda, d_grad, d_Hessian, d_pvec);
        getCRLB_7_2ch<<<grids, blcks>>>(NFits, boxsz, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T, d_xvec, d_CRLB);
    }

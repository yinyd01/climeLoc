...
/*!	@brief	Parameters in general
	...
	@param[in]	astigs:				(nchannels * 9) float, [PSFsigmax0, PSFsigmay0, shiftx, shifty, dof, Ax, Bx, Ay, By] the astigmatic parameters	
...
__device__ static void fconstruct_gauss3D(int nnum, int pxID, int boxsz, float* astigs, float* loc, float* psf, float* dpsf_dloc)
{
	...
	for (unsigned int n = 0; n < nnum; n++) {
		zx = loc[n * NDIM + 2] - shiftx;
		zy = loc[n * NDIM + 2] - shifty;
		...
		delta_xu = ((float)(pxID % boxsz) + 1.0f - loc[n * NDIM]) / sx;
		delta_yu = ((float)(pxID / boxsz) + 1.0f - loc[n * NDIM + 1]) / sy;
		delta_xl = ((float)(pxID % boxsz) - loc[n * NDIM]) / sx;
		delta_yl = ((float)(pxID / boxsz) - loc[n * NDIM + 1]) / sy;
		...
		if (dpsf_dloc) {
			...
			dpsf_dloc[n * NDIM + 2] = dPSFx_dsx * dsx_dz * PSFy + PSFx * dPSFy_dsy * dsy_dz;
		}
	}
	return;
}
...
__device__ static void init_maxJump_1ch(int nnum, int boxsz, int zrange, float* xvec, float* maxJump)
{
	const int vnum0 = NDIM + 1;
	for (int n = 0; n < nnum; n++) {
		maxJump[n * vnum0] = max(1.0f, 0.1f * (float)boxsz);
		maxJump[n * vnum0 + 1] = max(1.0f, 0.1f * (float)boxsz);
		maxJump[n * vnum0 + 2] = max(2.0f, 0.05f * (float)zrange);
...
__device__ static void boxConstrain_1ch(int nnum, int boxsz, int zrange, float* xvec)
{
	const int vnum0 = NDIM + 1;
	for (int n = 0; n < nnum; n++) {
		xvec[n * vnum0] = min(max(xvec[n * vnum0], -1.0f), (float)boxsz + 1.0f);
		xvec[n * vnum0 + 1] = min(max(xvec[n * vnum0 + 1], -1.0f), (float)boxsz + 1.0f);
		xvec[n * vnum0 + 2] = min(max(xvec[n * vnum0 + 2], 0.0f), (float)zrange);
...
__device__ static void get_lgH_1ch(int boxsz, float* dataim, float* varim, float* astigs, int nnum, float* xvec,
	float* loc, float* psf, float* dpsf_dloc, float* deriv1, float* loss, float* grad, float* Hessian)
{
	...
	*loss = 0.0f;
	...
	for (unsigned int pxID = 0; pxID < boxsz * boxsz; pxID++) {
		data = dataim[pxID];
		fconstruct_gauss3D(nnum, pxID, boxsz, astigs, loc, psf, dpsf_dloc);
...
__device__ static void get_FIM_1ch(int boxsz, float* varim, float* astigs, int nnum, float* xvec,
...
/*!	@brief	set the maxJump	*/
__device__ static void init_maxJump_2ch(int nnum, int boxsz, int zrange, float* xvec, float* maxJump)
{
...
/*!	@brief	box-constrain the xvec	*/
__device__ static void boxConstrain_2ch(int nnum, int boxsz, int zrange, float* xvec)
{
	const int vnum0 = NDIM + 2;
	for (int n = 0; n < nnum; n++) {
		xvec[n * vnum0] = min(max(xvec[n * vnum0], -1.0f), (float)boxsz + 1.0f);
...
/*!	@brief	calculate the loss, gradients, and Hessian	*/
__device__ static void get_lgH_2ch(int boxsz, float* dataim, float* varim, float* astigs, int nnum, float* xvec, 
	...
	float* loc, float* psf, float* dpsf_dloc, float* deriv1, float* loss, float* grad, float* Hessian)
{
	...
	*loss = 0.0f;
	...
	for (unsigned int chID = 0; chID < nchannels; chID++) {
		...
		p_astigs = astigs + chID * 9;
		for (unsigned int pxID = 0; pxID < boxsz * boxsz; pxID++) {
			data = dataim[chID * boxsz * boxsz + pxID];
			fconstruct_gauss3D(nnum, pxID, boxsz, p_astigs, loc, psf, dpsf_dloc);
...
/*!	@brief	calculate the FIM		*/
__device__ static void get_FIM_2ch(int boxsz, float* varim, float* astigs, int nnum, float* xvec, 
	int warpdeg, int* lu, float* coeff_R2T, float*loc_ref, float* trans_deriv,
	float* loc, float* psf, float* dpsf_dloc, float* deriv1, float* FIM)
{
	const int nchannels = 2, vnum0 = NDIM + nchannels;
...
*/
__device__ static void get_resPeakPos(int nchannels, int boxsz, int zrange, float* dataim, float* astigs, int nnum, float* xvec,
	float* loc, float* psf, float* x_resPeak, float* y_resPeak)
{
	...
	resp = 0.0f;
	for (unsigned int pxID = 0; pxID < boxsz * boxsz; pxID++) {
		fconstruct_gauss3D(nnum, pxID, boxsz, astigs, loc, psf, NULL);
		if (nchannels == 1)
			get_pixel_model_1ch(nnum, psf, xvec, &model);
		else

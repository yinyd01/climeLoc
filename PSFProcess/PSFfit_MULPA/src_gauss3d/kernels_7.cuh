...
#ifndef KERNELS_7_CUH
#define KERNELS_7_CUH

/*! @brief	kernel functions for dualview PSFfit_MULPA
	@param		nnum:				const int, 7, the number of emitters	
	...
	@param[in]	astigs:				(nchannels * 9) float, [PSFsigmax0, PSFsigmay0, shiftx, shifty, dof, Ax, Bx, Ay, By] the astigmatic parameters for each channel
	@param[in]	z_coarse:			(2) float, the z can be estimated by z_coarse[0] * (varx - vary) + z_coarse[1] for the 0-th channel
...
*/
__global__ void PushnPull_7(int NFits, int nchannels, int boxsz, int zrange, float* g_data, float* g_astigs, float* g_xvec)
{
	const int nnum = 7, vnum0 = NDIM + nchannels;
	const int vnum = nnum * vnum0 + nchannels;
	const float PSFsigmax = g_astigs[0], PSFsigmay = g_astigs[1];
	...
	if (Idx < NFits) {	
		...
		get_resPeakPos(nchannels, boxsz, zrange, dataim, g_astigs, nnum-1, xvec, loc_previous, psf, &x_resPeak, &y_resPeak);
		get_massCenter(nchannels, nnum-1, xvec, &x_massc, &y_massc);
		get_newLoc(boxsz, PSFsigmax, PSFsigmay, x_resPeak, y_resPeak, x_massc, y_massc, &x_new, &y_new);
		...
		for (unsigned int n = 0; n < nnum; n++)
			xvec[n * vnum0 + 2] = 0.5f * (float)zrange;
	}
	__syncthreads();
}
...
*/
__global__ void Init_7_1ch(int NFits, int boxsz, int zrange, float* g_data, float* g_var, float* astigs, float* g_xvec, 
	float* g_maxJump, float* g_Loss, float* g_grad, float* g_Hessian, float* g_pvec, float* g_lambda, char* g_iter_push)
{
	const int nnum = 7, nchannels = 1, vnum0 = NDIM + nchannels;	
	const int vnum = nnum * vnum0 + nchannels;
	unsigned int Idx = blockIdx.x * blockDim.x + threadIdx.x;
	...
	if (Idx < NFits) {
		...
		init_maxJump_1ch(nnum, boxsz, zrange, xvec, maxJump);

		// calculate the loss, gradient, and Hessian
		get_lgH_1ch(boxsz, dataim, varim, astigs, nnum, xvec, loc, psf, dpsf_dloc, deriv1, &Loss, grad, Hessian);
...
__global__ void Init_7_2ch(int NFits, int boxsz, int zrange, float* g_data, float* g_var, float* astigs, 
	int warpdeg, int* g_lu, float* coeff_R2T, float* g_xvec,  
	float* g_maxJump, float* g_Loss, float* g_grad, float* g_Hessian, float* g_pvec, float* g_lambda, char* g_iter_push)
{
	const int nnum = 7, nchannels = 2, vnum0 = NDIM + nchannels;
	const int vnum = nnum * vnum0 + nchannels;
	unsigned int Idx = blockIdx.x * blockDim.x + threadIdx.x;
	float *dataim = nullptr, *varim = nullptr;
	float *xvec = nullptr, *maxJump = nullptr;
	...
	if (Idx < NFits) {
		...
		// initialize the maxJump vector
		maxJump = g_maxJump + Idx * vnum;
		init_maxJump_2ch(nnum, boxsz, zrange, xvec, maxJump);
		
		// calculate the loss, gradient, and Hessian
		get_lgH_2ch(boxsz, dataim, varim, astigs, nnum, xvec, warpdeg, lu, coeff_R2T, loc_ref, trans_deriv,
...
__global__ void LMupdate_7_1ch(int NFits, char* g_iter_push, int boxsz, int zrange, float* g_data, float* g_var, float* astigs,
	float* g_xvec, float* g_Loss, float* g_maxJump, float* g_lambda, float* g_grad, float* g_Hessian, float* g_pvec)
{
	...
	if (push_flag) {
		...
		if (decomp_flag == 1) {
			...
			boxConstrain_1ch(nnum, boxsz, zrange, xvec);
...
__global__ void LMupdate_7_2ch(int NFits, char* g_iter_push, int boxsz, int zrange, float* g_data, float* g_var, float* astigs, 
	...
	float* g_xvec, float* g_Loss, float* g_maxJump, float* g_lambda, float* g_grad, float* g_Hessian, float* g_pvec)
{
	...
	if (push_flag) {
		...
		if (decomp_flag == 1) {
			...
			boxConstrain_2ch(nnum, boxsz, zrange, xvec);
...
__global__ void getCRLB_7_1ch(int NFits, int boxsz, float* g_var, float* astigs, float* g_xvec, float* g_CRLB)
{
	const int nnum = 7, nchannels = 1, vnum0 = NDIM + nchannels;
	const int vnum = nnum * vnum0 + nchannels;
	unsigned int Idx = blockIdx.x * blockDim.x + threadIdx.x;
	...
	if (Idx < NFits) {
		...
		get_FIM_1ch(boxsz, varim, astigs, nnum, xvec, loc, psf, dpsf_dloc, deriv1, FIM);
...
__global__ void getCRLB_7_2ch(int NFits, int boxsz, float* g_var, float* astigs, 
	int warpdeg, int* g_lu, float* coeff_R2T, float* g_xvec, float* g_CRLB)
{
	...
	if (Idx < NFits) {
		...
		get_FIM_2ch(boxsz, varim, astigs, nnum, xvec, warpdeg, lu, coeff_R2T, loc_ref, trans_deriv, loc, psf, dpsf_dloc, deriv1, FIM);

...
__global__ void PreInit(int NFits, int nchannels, int boxsz, int zrange, float* g_data, float* g_xvec)
{
	...
	if (Idx < NFits) {	
		...
		if (nchannels == 1)
			init_xvec_1ch(boxsz, zrange, dataim, xvec);
		else
			init_xvec_2ch(boxsz, zrange, dataim, xvec);
	}
	__syncthreads();
}
...
*/
__global__ void Init_1_1ch(int NFits, int boxsz, int zrange, float* g_data, float* g_var, float* astigs, float* g_xvec, 
...
__global__ void Init_1_2ch(int NFits, int boxsz, int zrange, float* g_data, float* g_var, float* astigs, 
...
__global__ void LMupdate_1_1ch(int NFits, char* g_iter_push, int boxsz, int zrange, float* g_data, float* g_var, float* astigs,
...
__global__ void LMupdate_1_2ch(int NFits, char* g_iter_push, int boxsz, int zrange, float* g_data, float* g_var, float* astigs, 
...
__global__ void getCRLB_1_1ch(int NFits, int boxsz, float* g_var, float* astigs, float* g_xvec, float* g_CRLB)
{
	const int nnum = 1, nchannels = 1, vnum0 = NDIM + nchannels;
	const int vnum = nnum * vnum0 + nchannels;
	unsigned int Idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void getCRLB_1_2ch(int NFits, int boxsz, float* g_var, float* astigs, 

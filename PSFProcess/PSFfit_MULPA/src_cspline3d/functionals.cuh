...
/*!	@brief	Parameters in general
	...
	@param[in]	offset_x:			(nnum) int, preallocated array for the x-offset translating the loc from the box coordination to the spline coordination
	@param[in] 	offset_y:			(nnum) int, preallocated array for the y-offset translating the loc from the box coordination to the spline coordination
	@param[in] 	offset_z:			(nnum) int, preallocated array for the z-offset translating the loc from the box coordination to the spline coordination
	@param[in]	deltaf:				(nnum * 64) float, preallocated array for the deltas for the corresponding cspline coefficients
	@param[in] 	ddeltaf_dx:			(nnum * 64) float, preallocated array for the deltas for the derivatives over the x-axis of the corresponding cspline coefficients
	@param[in] 	ddeltaf_dy:			(nnum * 64) float, preallocated array for the deltas for the derivatives over the y-axis of the corresponding cspline coefficients
	@param[in] 	ddeltaf_dz:			(nnum * 64) float, preallocated array for the deltas for the derivatives over the y-axis of the corresponding cspline coefficients
...
__device__ static void TransR2T(int nnum, int warpdeg, float* coeff_R2T, int* lu_ref, int* lu_tar, float* loc_ref, float* loc_tar, float* trans_deriv)
{
	...
	for (n = 0; n < nnum; n++) {
		...
		loc_tar[n * NDIM + 2] = loc_ref[n * NDIM + 2];
	}
	return;
}
...
/*!	@brief	Translating the actural pixels idx to spline idx by mapping the emitter center to the origin of the spline coordinates;	
			...
			[[[dz^0 * dy^0 * dx^0, ..., dz^0 * dy^0 * dx^3], [dz^0 * dy^1 * dx^0, ..., dz^0 * dy^1 * dx^3], ..., [dz^0 * dy^3 * dx^0, ..., dz^0 * dy^3 * dx^3]],
			  [dz^1 * dy^0 * dx^0, ..., dz^1 * dy^0 * dx^3], [dz^1 * dy^1 * dx^0, ..., dz^1 * dy^1 * dx^3], ..., [dz^1 * dy^3 * dx^0, ..., dz^1 * dy^3 * dx^3]],
			  ...
			  [dz^3 * dy^0 * dx^0, ..., dz^3 * dy^0 * dx^3], [dz^3 * dy^1 * dx^0, ..., dz^3 * dy^1 * dx^3], ..., [dz^3 * dy^3 * dx^0, ..., dz^3 * dy^3 * dx^3]]]
			It matches the coeff a_xyz at an cspline_idx[idz, idy, idx] as:
			[[[a_000, a_001, ..., a_003], [a_010, a_011, ..., a_013], ..., [a_030, a_031, ..., a_033]],
			 [[a_100, a_101, ..., a_103], [a_110, a_111, ..., a_013], ..., [a_130, a_131, ..., a_133]],
			 ...
			 [[a_300, a_301, ..., a_303], [a_310, a_311, ..., a_313], ..., [a_330, a_331, ..., a_333]]]		
*/
__device__ static void DeltaConstruct3D(int nnum, int splineszx, int splineszz, float* loc, 
	int* offset_x, int* offset_y, int* offset_z, float* deltaf, float* ddeltaf_dx, float* ddeltaf_dy, float* ddeltaf_dz)
{
	...
	for (n = 0; n < nnum; n++) {
		offset_x[n] = splineszx / 2 - (int)floor(loc[n * NDIM]);
		offset_y[n] = splineszx / 2 - (int)floor(loc[n * NDIM + 1]);
		offset_z[n] = splineszz / 2 - (int)floor(loc[n * NDIM + 2]);
		
		delta_x = 1.0f - (loc[n * NDIM] - floor(loc[n * NDIM]));
		delta_y = 1.0f - (loc[n * NDIM + 1] - floor(loc[n * NDIM + 1]));
		delta_z = 1.0f - (loc[n * NDIM + 2] - floor(loc[n * NDIM + 2]));
		for (i = 0, cz = 1.0f; i < 4; i++) {
			for (j = 0, cy = 1.0f; j < 4; j++) {
				for (k = 0, cx = 1.0f; k < 4; k++) {
					deltaf[n*64 + i*16 + j*4 + k] = cz * cy * cx;
					if (ddeltaf_dz && ddeltaf_dy && ddeltaf_dx) {
						if (k < 3) ddeltaf_dx[n*64 + i*16 + j*4 + k+1] = ((float)k + 1) * cz * cy * cx;
						if (j < 3) ddeltaf_dy[n*64 + i*16 + (j+1)*4 + k] = ((float)j + 1) * cz * cy * cx;
						if (i < 3) ddeltaf_dz[n*64 + (i+1)*16 + j*4 + k] = ((float)i + 1) * cz * cy * cx;
						if (k == 3) ddeltaf_dx[n*64 + i*16 + j*4] = 0.0f;
						if (j == 3) ddeltaf_dy[n*64 + i*16 + k] = 0.0f;
						if (i == 3) ddeltaf_dz[n*64 + j*4 + k] = 0.0f;	
					}
					cx *= delta_x;
				}
				cy *= delta_y;
...
/*!	@brief	Construct the psf value of each emitter, and their derivatives over their locations at a pxID.	*/
__device__ static void fconstruct_spline3D(int nnum, int pxID, int boxsz, int splineszx, int splineszz, float* coeff_PSF, 
	int* offset_x, int* offset_y, int* offset_z, float* deltaf, float* ddeltaf_dx, float* ddeltaf_dy, float* ddeltaf_dz, float* psf, float* dpsf_dloc)
{
	float dum = 0.0f;
	int n = 0, spindx = 0, spindy = 0, spindz = 0, splineIdx = 0, i = 0;
	
	memset(psf, 0, nnum * sizeof(float));
	if (dpsf_dloc && ddeltaf_dx && ddeltaf_dy && ddeltaf_dz)
		memset(dpsf_dloc, 0, nnum * NDIM * sizeof(float));

	for (n = 0; n < nnum; n++) {
		spindx = (pxID % boxsz) + offset_x[n];
		spindy = (pxID / boxsz) + offset_y[n];
		spindz = splineszz / 2 + offset_z[n];
		...
		for (i = 0, dum = 0.0f; i < 64; i++) {
			dum += deltaf[n * 64 + i] * coeff_PSF[splineIdx * 64 + i];
			if (dpsf_dloc && ddeltaf_dx && ddeltaf_dy && ddeltaf_dz) {
				dpsf_dloc[n * NDIM] -= ddeltaf_dx[n * 64 + i] * coeff_PSF[splineIdx * 64 + i];
				dpsf_dloc[n * NDIM + 1] -= ddeltaf_dy[n * 64 + i] * coeff_PSF[splineIdx * 64 + i];
				dpsf_dloc[n * NDIM + 2] -= ddeltaf_dz[n * 64 + i] * coeff_PSF[splineIdx * 64 + i];
			}
		}
		psf[n] = max(dum, FLT_EPSILON);
	}
	return;
}
...
__device__ static void init_maxJump_1ch(int nnum, int boxsz, int splineszz, float* xvec, float* maxJump)
{
	const int vnum0 = NDIM + 1;
	for (int n = 0; n < nnum; n++) {
		maxJump[n * vnum0] = max(1.0f, 0.1f * (float)boxsz);
		maxJump[n * vnum0 + 1] = max(1.0f, 0.1f * (float)boxsz);
		maxJump[n * vnum0 + 2] = max(2.0f, 0.05f * (float)splineszz);
...
__device__ static void boxConstrain_1ch(int nnum, int boxsz, int splineszz, float* xvec)
{
	const int vnum0 = NDIM + 1;
	for (int n = 0; n < nnum; n++) {
		xvec[n * vnum0] = min(max(xvec[n * vnum0], -1.0f), (float)boxsz + 1.0f);
		xvec[n * vnum0 + 1] = min(max(xvec[n * vnum0 + 1], -1.0f), (float)boxsz + 1.0f);
		xvec[n * vnum0 + 2] = min(max(xvec[n * vnum0 + 2], 0.0f), (float)splineszz);
...
__device__ static void get_lgH_1ch(int boxsz, int splineszx, int splineszz, float* dataim, float* varim, float* coeff_PSF, int nnum, float* xvec,
	float* loc, int* offset_x, int* offset_y, int* offset_z, float* deltaf, float* ddeltaf_dx, float* ddeltaf_dy, float* ddeltaf_dz, float* psf, float* dpsf_dloc, float* deriv1,
	float* loss, float* grad, float* Hessian)
{
	...
	*loss = 0.0f;
	...
	DeltaConstruct3D(nnum, splineszx, splineszz, loc, offset_x, offset_y, offset_z, deltaf, ddeltaf_dx, ddeltaf_dy, ddeltaf_dz);
	for (unsigned int pxID = 0; pxID < boxsz * boxsz; pxID++) {
		data = dataim[pxID];
		fconstruct_spline3D(nnum, pxID, boxsz, splineszx, splineszz, coeff_PSF, offset_x, offset_y, offset_z, deltaf, ddeltaf_dx, ddeltaf_dy, ddeltaf_dz, psf, dpsf_dloc);
...
__device__ static void get_FIM_1ch(int boxsz, int splineszx, int splineszz, float* varim, float* coeff_PSF, int nnum, float* xvec,
	float* loc, int* offset_x, int* offset_y, int* offset_z, float* deltaf, float* ddeltaf_dx, float* ddeltaf_dy, float* ddeltaf_dz, float* psf, float* dpsf_dloc, float* deriv1, float* FIM)
{
	const int nchannels = 1, vnum0 = NDIM + nchannels;
	const int vnum = nnum * vnum0 + nchannels;
	float model = 0.0f;
...
/*!	@brief	set the maxJump		*/
__device__ static void init_maxJump_2ch(int nnum, int boxsz, int splineszz, float* xvec, float* maxJump)
{
...
/*!	@brief	box-constrain the xvec	*/
__device__ static void boxConstrain_2ch(int nnum, int boxsz, int splineszz, float* xvec)
{
	const int vnum0 = NDIM + 2;
	for (int n = 0; n < nnum; n++) {
		xvec[n * vnum0] = min(max(xvec[n * vnum0], -1.0f), (float)boxsz + 1.0f);
...
__device__ static void get_pixel_deriv_2ch(int nnum, int chID, float* psf, float* dpsf_dloc, float* trans_deriv_R2T, float* xvec, float* deriv1)
{
	...
	for (int n = 0; n < nnum; n++) {
		...
		deriv1[n * vnum0 + 2] = xvec[n * vnum0 + NDIM] * dum_fracN * dpsf_dloc[n * NDIM + 2];
...
/*!	@brief	calculate the loss, gradients, and Hessian	*/
__device__ static void get_lgH_2ch(int boxsz, int splineszx, int splineszz, float* dataim, float* varim, float* coeff_PSF, int nnum, float* xvec, 
	...
	*loss = 0.0f;
	...
	for (unsigned int chID = 0; chID < nchannels; chID++) {
		...
		for (unsigned int pxID = 0; pxID < boxsz * boxsz; pxID++) {
			...
			fconstruct_spline3D(nnum, pxID, boxsz, splineszx, splineszz, p_coeff_PSF, offset_x, offset_y, offset_z, deltaf, ddeltaf_dx, ddeltaf_dy, ddeltaf_dz, psf, dpsf_dloc);
...
/*!	@brief	calculate the FIM		*/
__device__ static void get_FIM_2ch(int boxsz, int splineszx, int splineszz, float* varim, float* coeff_PSF, int nnum, float* xvec, 
	int warpdeg, int* lu, float* coeff_R2T, float*loc_ref, float* trans_deriv,
	float* loc, int* offset_x, int* offset_y, int* offset_z, float* deltaf, float* ddeltaf_dx, float* ddeltaf_dy, float* ddeltaf_dz, float* psf, float* dpsf_dloc, float* deriv1, float* FIM)
{
	const int nchannels = 2, vnum0 = NDIM + nchannels;
...
/*!	@brief	set the maxJump		*/
__device__ static void init_maxJump_BP(int nnum, int boxsz, int splineszz, float* xvec, float* maxJump)
{
	const int vnum0 = NDIM + 1;
	for (int n = 0; n < nnum; n++) {
		maxJump[n * vnum0] = max(1.0f, 0.1f * (float)boxsz);
...
/*!	@brief	box-constrain the xvec	*/
__device__ static void boxConstrain_BP(int nnum, int boxsz, int splineszz, float* xvec)
{
	const int vnum0 = NDIM + 1;
	for (int n = 0; n < nnum; n++) {
		xvec[n * vnum0] = min(max(xvec[n * vnum0], -1.0f), (float)boxsz + 1.0f);
...
/*!	@brief	calculate the model value at a pixel	*/
__device__ static void get_pixel_model_BP(int nnum, float* psf, float* xvec, float* model)
{
	const int vnum0 = NDIM + 1;
	*model = 0.0f;
	for (int n = 0; n < nnum; n++) {
		*model += xvec[n * vnum0 + NDIM] * 0.5f * psf[n];
	}
	*model += xvec[nnum * vnum0];
	*model = max(*model, FLT_EPSILON);
	return;
...
/*!	@brief	calculate the derivatives at a pixel	*/
__device__ static void get_pixel_deriv_BP(int nnum, float* psf, float* dpsf_dloc, float* trans_deriv_R2T, float* xvec, float* deriv1)
{
	const int vnum0 = NDIM + 1;
	memset(deriv1, 0, (nnum * vnum0 + 1) * sizeof(float));
	for (int n = 0; n < nnum; n++) {
		deriv1[n * vnum0] = xvec[n * vnum0 + NDIM] * 0.5f * (dpsf_dloc[n * NDIM] * trans_deriv_R2T[n * 4] + dpsf_dloc[n * NDIM + 1] * trans_deriv_R2T[n * 4 + 1]);
		deriv1[n * vnum0 + 1] = xvec[n * vnum0 + NDIM] * 0.5f * (dpsf_dloc[n * NDIM] * trans_deriv_R2T[n * 4 + 2] + dpsf_dloc[n * NDIM + 1] * trans_deriv_R2T[n * 4 + 3]);
		deriv1[n * vnum0 + 2] = xvec[n * vnum0 + NDIM] * 0.5f * dpsf_dloc[n * NDIM + 2];
		deriv1[n * vnum0 + NDIM] = 0.5f * psf[n];
	}
	deriv1[nnum * vnum0] = 1.0f;
	return;
}
...
/*!	@brief	calculate the loss, gradients, and Hessian	*/
__device__ static void get_lgH_BP(int boxsz, int splineszx, int splineszz, float* dataim, float* varim, float* coeff_PSF, int nnum, float* xvec, 
	int warpdeg, int* lu, float* coeff_R2T, float*loc_ref, float* trans_deriv,
	float* loc, int* offset_x, int* offset_y, int* offset_z, float* deltaf, float* ddeltaf_dx, float* ddeltaf_dy, float* ddeltaf_dz, float* psf, float* dpsf_dloc, float* deriv1,
	float* loss, float* grad, float* Hessian)
{
	...
	*loss = 0.0f;
	...
	for (unsigned int chID = 0; chID < nchannels; chID++) {
		...
		for (unsigned int pxID = 0; pxID < boxsz * boxsz; pxID++) {
			data = dataim[chID * boxsz * boxsz + pxID];
			fconstruct_spline3D(nnum, pxID, boxsz, splineszx, splineszz, p_coeff_PSF, offset_x, offset_y, offset_z, deltaf, ddeltaf_dx, ddeltaf_dy, ddeltaf_dz, psf, dpsf_dloc);
			get_pixel_model_BP(nnum, psf, xvec, &model);
			get_pixel_deriv_BP(nnum, psf, dpsf_dloc, trans_deriv, xvec, deriv1);
			if (varim) {
				data += varim[chID * boxsz * boxsz + pxID];
				model += varim[chID * boxsz * boxsz + pxID];
			}	
...
/*!	@brief	calculate the FIM		*/
__device__ static void get_FIM_BP(int boxsz, int splineszx, int splineszz, float* varim, float* coeff_PSF, int nnum, float* xvec, 
	int warpdeg, int* lu, float* coeff_R2T, float*loc_ref, float* trans_deriv,
	float* loc, int* offset_x, int* offset_y, int* offset_z, float* deltaf, float* ddeltaf_dx, float* ddeltaf_dy, float* ddeltaf_dz, float* psf, float* dpsf_dloc, float* deriv1, float* FIM)
{
	const int nchannels = 2, vnum0 = NDIM + 1;
...
*/
__device__ static void get_resPeakPos_1ch(int boxsz, int splineszx, int splineszz, float* dataim, float* coeff_PSF, int nnum, float* xvec,
	float* loc, int* offset_x, int* offset_y, int* offset_z, float* deltaf, float* psf, float* x_resPeak, float* y_resPeak)
{
	const int vnum0 = NDIM + 1;
	...
	resp = 0.0f;
	DeltaConstruct3D(nnum, splineszx, splineszz, loc, offset_x, offset_y, offset_z, deltaf, NULL, NULL, NULL);
	for (unsigned int pxID = 0; pxID < boxsz * boxsz; pxID++) {
		fconstruct_spline3D(nnum, pxID, boxsz, splineszx, splineszz, coeff_PSF, offset_x, offset_y, offset_z, deltaf, NULL, NULL, NULL, psf, NULL);
		get_pixel_model_1ch(nnum, psf, xvec, &model);
...
/*!	@brief	get thee peak position of the residual image
	...
	@param[out] y_resPeak: 		(1), float, pointer to the y-axis residual peak position
*/
__device__ static void get_resPeakPos_2ch(int boxsz, int splineszx, int splineszz, float* dataim, float* coeff_PSF, int nnum, float* xvec,
	float* loc, int* offset_x, int* offset_y, int* offset_z, float* deltaf, float* psf, float* x_resPeak, float* y_resPeak)
{
	const int vnum0 = NDIM + 2;
	float model = 0.0f, resp = 0.0f;
...
/*!	@brief	get thee peak position of the residual image
	...
	@param[out] y_resPeak: 		(1), float, pointer to the y-axis residual peak position
*/
__device__ static void get_resPeakPos_BP(int boxsz, int splineszx, int splineszz, float* dataim, float* coeff_PSF, int nnum, float* xvec,
	float* loc, int* offset_x, int* offset_y, int* offset_z, float* deltaf, float* psf, float* x_resPeak, float* y_resPeak)
{
	const int vnum0 = NDIM + 1;
	float model = 0.0f, resp = 0.0f;
...
*/
__device__ static void get_massCenter_1ch(int nnum, float* xvec, float* x_mc, float* y_mc)
{
	int vnum0 = NDIM + 1, n = 0;
	float photon = 0.0f, tmpxc = 0.0f, tmpyc = 0.0f, tmpI = 0.0f;

	for (n = 0, tmpxc = 0.0f, tmpyc = 0.0f, tmpI = 0.0f; n < nnum; n++) {
		photon = max(xvec[n * vnum0 + NDIM], 1.0f);
...
/*!	@brief	get the mass center
	...
	@param[out] y_mc: 			(1), float, pointer to the y-axis mass center
*/
__device__ static void get_massCenter_2ch(int nnum, float* xvec, float* x_mc, float* y_mc)
{
	int vnum0 = NDIM + 2, n = 0;
	float photon = 0.0f, tmpxc = 0.0f, tmpyc = 0.0f, tmpI = 0.0f;

	for (n = 0, tmpxc = 0.0f, tmpyc = 0.0f, tmpI = 0.0f; n < nnum; n++) {
		photon = max(xvec[n * vnum0 + NDIM] * xvec[n * vnum0 + NDIM + 1], 1.0f);
		tmpxc += photon * xvec[n * vnum0];
		tmpyc += photon * xvec[n * vnum0 + 1];
		tmpI += photon;
	}
...
/*!	@brief	get the mass center
	...
	@param[out] y_mc: 			(1), float, pointer to the y-axis mass center
*/
__device__ static void get_massCenter_BP(int nnum, float* xvec, float* x_mc, float* y_mc)
{
	int vnum0 = NDIM + 1, n = 0;
	float photon = 0.0f, tmpxc = 0.0f, tmpyc = 0.0f, tmpI = 0.0f;

	for (n = 0, tmpxc = 0.0f, tmpyc = 0.0f, tmpI = 0.0f; n < nnum; n++) {
		photon = max(xvec[n * vnum0 + NDIM] * 0.5f, 1.0f);
		tmpxc += photon * xvec[n * vnum0];
		tmpyc += photon * xvec[n * vnum0 + 1];
		tmpI += photon;
	}
...
/*!	@brief	get the nearest neighbor from the xvec for the input location
	@param[in]	channel_mod:	int, 0 for 1-channel, 1 for 2-channel, 2 for biplane
...
*/
__device__ static void get_nn(int channel_mod, int nnum, float* xvec, float x_in, float y_in, int* ind, float* nnd)
{
	int vnum0 = (channel_mod == 1) ? NDIM + 2 : NDIM + 1;
	float dist2 = 0.0f;
...
*/
__device__ static void get_newLoc(int boxsz, float x_resPeak, float y_resPeak, float x_massc, float y_massc, float* x_new, float* y_new)
{
	const float pplen = 0.5f * PSFSIG; 
	float dist = 0.0f;
	
	// push and pull
	dist = sqrt((x_massc - x_resPeak) * (x_massc - x_resPeak) + (y_massc - y_resPeak) * (y_massc - y_resPeak));
	if ((x_resPeak < PSFSIG) || (x_resPeak > (float)boxsz - PSFSIG) || (y_resPeak < PSFSIG) || (y_resPeak > (float)boxsz - PSFSIG)) {

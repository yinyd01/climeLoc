...
/*! @brief	kernel functions for dualview PSFfit_MULPA
	@param		nnum:				const int, 3, the number of emitters
...
*/
__global__ void PushnPull_3_1ch(int NFits, int boxsz, int splineszx, int splineszz, float* g_data, float* coeff_PSF, float* g_xvec)
{
	const int nnum = 3, nchannels = 1, vnum0 = NDIM + 1;
...
/*! @brief	<<<ceil(NFits/BLCKSZ), BLCKSZ = 64>>>
			Push-and-Pull for the nnum-th xvec from the given (nnum-1)-th xvec, which are both stored in xvec_pool
*/
__global__ void PushnPull_3_2ch(int NFits, int boxsz, int splineszx, int splineszz, float* g_data, float* coeff_PSF, float* g_xvec)
{
	const int nnum = 3, nchannels = 2, vnum0 = NDIM + 2;
...
/*! @brief	<<<ceil(NFits/BLCKSZ), BLCKSZ = 64>>>
			Push-and-Pull for the nnum-th xvec from the given (nnum-1)-th xvec, which are both stored in xvec_pool
*/
__global__ void PushnPull_3_BP(int NFits, int boxsz, int splineszx, int splineszz, float* g_data, float* coeff_PSF, float* g_xvec)
{
	const int nnum = 3, nchannels = 2, vnum0 = NDIM + 1;
	const int vnum = nnum * vnum0 + 1;
	unsigned int Idx = blockIdx.x * blockDim.x + threadIdx.x;
	float *dataim = nullptr, *xvec = nullptr;
...
*/
__global__ void Init_3_1ch(int NFits, int boxsz, int splineszx, int splineszz, float* g_data, float* g_var, float* coeff_PSF, float* g_xvec, 
...
__global__ void Init_3_2ch(int NFits, int boxsz, int splineszx, int splineszz, float* g_data, float* g_var, float* coeff_PSF, 
...
__global__ void Init_3_BP(int NFits, int boxsz, int splineszx, int splineszz, float* g_data, float* g_var, float* coeff_PSF, 
	int warpdeg, int* g_lu, float* coeff_R2T, float* g_xvec, 
	float* g_maxJump, float* g_Loss, float* g_grad, float* g_Hessian, float* g_pvec, float* g_lambda, char* g_iter_push)
{
	const int nnum = 3, nchannels = 2, vnum0 = NDIM + 1;
...
__global__ void LMupdate_3_1ch(int NFits, char* g_iter_push, int boxsz, int splineszx, int splineszz, float* g_data, float* g_var, float* coeff_PSF,
...
__global__ void LMupdate_3_2ch(int NFits, char* g_iter_push, int boxsz, int splineszx, int splineszz, float* g_data, float* g_var, float* coeff_PSF, 
...
__global__ void LMupdate_3_BP(int NFits, char* g_iter_push, int boxsz, int splineszx, int splineszz, float* g_data, float* g_var, float* coeff_PSF, 
	int warpdeg, int* g_lu, float* coeff_R2T, 
	float* g_xvec, float* g_Loss, float* g_maxJump, float* g_lambda, float* g_grad, float* g_Hessian, float* g_pvec)
{
	const int nnum = 3, nchannels = 2, vnum0 = NDIM + 1;
...
__global__ void getCRLB_3_1ch(int NFits, int boxsz, int splineszx, int splineszz, float* g_var, float* coeff_PSF, float* g_xvec, float* g_CRLB)
{
	const int nnum = 3, nchannels = 1, vnum0 = NDIM + nchannels;
	const int vnum = nnum * vnum0 + nchannels;
	unsigned int Idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void getCRLB_3_2ch(int NFits, int boxsz, int splineszx, int splineszz, float* g_var, float* coeff_PSF, 
...
__global__ void getCRLB_3_BP(int NFits, int boxsz, int splineszx, int splineszz, float* g_var, float* coeff_PSF, 
	int warpdeg, int* g_lu, float* coeff_R2T, float* g_xvec, float* g_CRLB)
{
	const int nnum = 3, nchannels = 2, vnum0 = NDIM + 1;
	const int vnum = nnum * vnum0 + 1;

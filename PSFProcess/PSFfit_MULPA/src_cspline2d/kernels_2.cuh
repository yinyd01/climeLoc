...
/*! @brief	kernel functions for dualview PSFfit_MULPA
	@param		nnum:				const int, 2, the number of emitters 
	...
	@param[in]	coeff_PSF: 			(nchannels * splinesz * splinesz * 16) float, spline coefficient for the nchannel 2d PSF
...
*/
__global__ void PushnPull_2(int NFits, int nchannels, int boxsz, int splinesz, float* g_data, float* coeff_PSF, float* g_xvec)
{
...
*/
__global__ void Init_2_1ch(int NFits, int boxsz, int splinesz, float* g_data, float* g_var, float* coeff_PSF, float* g_xvec,
...
__global__ void Init_2_2ch(int NFits, int boxsz, int splinesz, float* g_data, float* g_var, float* coeff_PSF, 
...
__global__ void LMupdate_2_1ch(int NFits, char* g_iter_push, int boxsz, int splinesz, float* g_data, float* g_var, float* coeff_PSF,
...
__global__ void LMupdate_2_2ch(int NFits, char* g_iter_push, int boxsz, int splinesz, float* g_data, float* g_var, float* coeff_PSF, 
...
__global__ void getCRLB_2_1ch(int NFits, int boxsz, int splinesz, float* g_var, float* coeff_PSF, float* g_xvec, float* g_CRLB)
{
	const int nnum = 2, nchannels = 1, vnum0 = NDIM + nchannels;
	const int vnum = nnum * vnum0 + nchannels;
	unsigned int Idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void getCRLB_2_2ch(int NFits, int boxsz, int splinesz, float* g_var, float* coeff_PSF, 

...
/*!	@brief	Parameters in general
	...
	@param[in]	coeff_PSF:			(nchannels * splineszx * splineszx * 16) float, cubic spline coefficients of the experimental PSF
...
/*!	@brief	Translating the actural pixels idx to spline idx by mapping the emitter center to the origin of the spline coordinates;	
			...
			It matches the coeff a_xyz at an cspline_idx[idy, idx] as:
				[[a_01, ..., a_03], [a_10, ..., a_13], ..., [a_31, ..., a_33]]	
*/
__device__ static void DeltaConstruct2D(int nnum, int splinesz, float* loc, int* offset_x, int* offset_y, float* deltaf, float* ddeltaf_dx, float* ddeltaf_dy)
{
	float delta_x = 0.0f, delta_y = 0.0f, cx = 1.0f, cy = 1.0f;
	unsigned int n = 0, i = 0, j = 0;

	for (n = 0; n < nnum; n++) {
		offset_x[n] = splinesz / 2 - (int)floor(loc[n * NDIM]);
		offset_y[n] = splinesz / 2 - (int)floor(loc[n * NDIM + 1]);
		...
		for (i = 0, cy = 1.0f; i < 4; i++) {
			for (j = 0, cx = 1.0f; j < 4; j++) {
				deltaf[n * 16 + i * 4 + j] = cy * cx;
				if (ddeltaf_dx && ddeltaf_dy) {
					if (j < 3) ddeltaf_dx[n * 16 + i * 4 + j + 1] = ((float)j + 1) * cy * cx;
					if (i < 3) ddeltaf_dy[n * 16 + (i + 1) * 4 + j] = ((float)i + 1) * cy * cx;
					if (j == 3) ddeltaf_dx[n * 16 + i * 4] = 0.0f;
					if (i == 3) ddeltaf_dy[n * 16 + j] = 0.0f;
				}
				cx *= delta_x;
			}
			cy *= delta_y;
...
__device__ static void fconstruct_spline2D(int nnum, int pxID, int boxsz, int splinesz, float* coeff_PSF, 
	int* offset_x, int* offset_y, float* deltaf, float* ddeltaf_dx, float* ddeltaf_dy, float* psf, float* dpsf_dloc)
{
	float dum = 0.0f;
	int n = 0, spindx = 0, spindy = 0, splineIdx = 0, i = 0;
	...
	for (n = 0; n < nnum; n++) {
		...
		for (i = 0, dum = 0.0f; i < 16; i++) {
			dum += deltaf[n * 16 + i] * coeff_PSF[splineIdx * 16 + i];
			if (dpsf_dloc && ddeltaf_dx && ddeltaf_dy) {
				dpsf_dloc[n * NDIM] -= ddeltaf_dx[n * 16 + i] * coeff_PSF[splineIdx * 16 + i];
				dpsf_dloc[n * NDIM + 1] -= ddeltaf_dy[n * 16 + i] * coeff_PSF[splineIdx * 16 + i];
			}
		}
...
__device__ static void get_lgH_1ch(int boxsz, int splineszx, float* dataim, float* varim, float* coeff_PSF, int nnum, float* xvec,
	float* loc, int* offset_x, int* offset_y, float* deltaf, float* ddeltaf_dx, float* ddeltaf_dy, float* psf, float* dpsf_dloc, float* deriv1,
	float* loss, float* grad, float* Hessian)
{
	...
	*loss = 0.0f;
	...
	DeltaConstruct2D(nnum, splineszx, loc, offset_x, offset_y, deltaf, ddeltaf_dx, ddeltaf_dy);
	for (unsigned int pxID = 0; pxID < boxsz * boxsz; pxID++) {
		data = dataim[pxID];
		fconstruct_spline2D(nnum, pxID, boxsz, splineszx, coeff_PSF, offset_x, offset_y, deltaf, ddeltaf_dx, ddeltaf_dy, psf, dpsf_dloc);
...
__device__ static void get_FIM_1ch(int boxsz, int splineszx, float* varim, float* coeff_PSF, int nnum, float* xvec,
	float* loc, int* offset_x, int* offset_y, float* deltaf, float* ddeltaf_dx, float* ddeltaf_dy, float* psf, float* dpsf_dloc, float* deriv1, float* FIM)
{
	const int nchannels = 1, vnum0 = NDIM + nchannels;
	const int vnum = nnum * vnum0 + nchannels;
	float model = 0.0f;
...
/*!	@brief	calculate the loss, gradients, and Hessian	*/
__device__ static void get_lgH_2ch(int boxsz, int splineszx, float* dataim, float* varim, float* coeff_PSF, int nnum, float* xvec, 
	...
	float* loss, float* grad, float* Hessian)
{
	...
	*loss = 0.0f;
	...
	for (unsigned int chID = 0; chID < nchannels; chID++) {
		...
		p_coeff_PSF = coeff_PSF + chID * splineszx * splineszx * 16;
		DeltaConstruct2D(nnum, splineszx, loc, offset_x, offset_y, deltaf, ddeltaf_dx, ddeltaf_dy);
		for (unsigned int pxID = 0; pxID < boxsz * boxsz; pxID++) {
			data = dataim[chID * boxsz * boxsz + pxID];
			fconstruct_spline2D(nnum, pxID, boxsz, splineszx, p_coeff_PSF, offset_x, offset_y, deltaf, ddeltaf_dx, ddeltaf_dy, psf, dpsf_dloc);
...
/*!	@brief	calculate the FIM		*/
__device__ static void get_FIM_2ch(int boxsz, int splineszx, float* varim, float* coeff_PSF, int nnum, float* xvec, 
	int warpdeg, int* lu, float* coeff_R2T, float*loc_ref, float* trans_deriv,
	float* loc, int* offset_x, int* offset_y, float* deltaf, float* ddeltaf_dx, float* ddeltaf_dy, float* psf, float* dpsf_dloc, float* deriv1, float* FIM)
{
	const int nchannels = 2, vnum0 = NDIM + nchannels;
...
*/
__device__ static void get_resPeakPos(int nchannels, int boxsz, int splineszx, float* dataim, float* coeff_PSF, int nnum, float* xvec,
	float* loc, int* offset_x, int* offset_y, float* deltaf, float* psf, float* x_resPeak, float* y_resPeak)
{
	...
	resp = 0.0f;
	DeltaConstruct2D(nnum, splineszx, loc, offset_x, offset_y, deltaf, NULL, NULL);
	for (unsigned int pxID = 0; pxID < boxsz * boxsz; pxID++) {
		fconstruct_spline2D(nnum, pxID, boxsz, splineszx, coeff_PSF, offset_x, offset_y, deltaf, NULL, NULL, psf, NULL);
		if (nchannels == 1)
			get_pixel_model_1ch(nnum, psf, xvec, &model);
		else

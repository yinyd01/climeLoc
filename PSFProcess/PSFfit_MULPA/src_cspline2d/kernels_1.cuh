...
/*! @brief	kernel functions for dualview PSFfit_MULPA
	...
	@param		g_llrthresh:		(NMAX) float, the llr threshold at different nnum model (calculated with a given pval outside the kernel)	
*/
...
*/
__global__ void Init_1_1ch(int NFits, int boxsz, int splinesz, float* g_data, float* g_var, float* coeff_PSF, float* g_xvec,
...
__global__ void Init_1_2ch(int NFits, int boxsz, int splinesz, float* g_data, float* g_var, float* coeff_PSF, 
...
__global__ void LMupdate_1_1ch(int NFits, char* g_iter_push, int boxsz, int splinesz, float* g_data, float* g_var, float* coeff_PSF,
...
__global__ void LMupdate_1_2ch(int NFits, char* g_iter_push, int boxsz, int splinesz, float* g_data, float* g_var, float* coeff_PSF, 
...
__global__ void getCRLB_1_1ch(int NFits, int boxsz, int splinesz, float* g_var, float* coeff_PSF, float* g_xvec, float* g_CRLB)
{
	const int nnum = 1, nchannels = 1, vnum0 = NDIM + nchannels;
	const int vnum = nnum * vnum0 + nchannels;
	unsigned int Idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void getCRLB_1_2ch(int NFits, int boxsz, int splinesz, float* g_var, float* coeff_PSF, 

...
*/
__global__ void PushnPull_7(int NFits, int nchannels, int boxsz, int splinesz, float* g_data, float* coeff_PSF, float* g_xvec)
{
	...
	int offset_x[nnum - 1] = {0}, offset_y[nnum - 1] = {0};
	float loc_previous[(nnum - 1) * NDIM] = {0.0f}, deltaf[(nnum - 1) * 16] = {0.0f}, psf[nnum - 1] = {0.0f};
	...
	if (Idx < NFits) {	
		...
		get_resPeakPos(nchannels, boxsz, splinesz, dataim, coeff_PSF, nnum-1, xvec,
			loc_previous, offset_x, offset_y, deltaf, psf, &x_resPeak, &y_resPeak);
...
*/
__global__ void Init_7_1ch(int NFits, int boxsz, int splinesz, float* g_data, float* g_var, float* coeff_PSF, float* g_xvec,
	float* g_maxJump, float* g_Loss, float* g_grad, float* g_Hessian, float* g_pvec, float* g_lambda, char* g_iter_push)
{
	...
	int offset_x[nnum] = {0}, offset_y[nnum] = {0};
	float deltaf[nnum * 16] = {0.0f}, ddeltaf_dx[nnum * 16] = {0.0f}, ddeltaf_dy[nnum * 16] = {0.0f};
	...
	if (Idx < NFits) {
		...
		get_lgH_1ch(boxsz, splinesz, dataim, varim, coeff_PSF, nnum, xvec,
			loc, offset_x, offset_y, deltaf, ddeltaf_dx, ddeltaf_dy, psf, dpsf_dloc, deriv1,
			&Loss, grad, Hessian);
...
__global__ void Init_7_2ch(int NFits, int boxsz, int splinesz, float* g_data, float* g_var, float* coeff_PSF, 
	...
	float* g_maxJump, float* g_Loss, float* g_grad, float* g_Hessian, float* g_pvec, float* g_lambda, char* g_iter_push)
{
	...
	if (Idx < NFits) {
		...
		// calculate the loss, gradient, and Hessian
		get_lgH_2ch(boxsz, splinesz, dataim, varim, coeff_PSF, nnum, xvec, 
...
__global__ void LMupdate_7_1ch(int NFits, char* g_iter_push, int boxsz, int splinesz, float* g_data, float* g_var, float* coeff_PSF,
...
__global__ void LMupdate_7_2ch(int NFits, char* g_iter_push, int boxsz, int splinesz, float* g_data, float* g_var, float* coeff_PSF, 
	...
	if (push_flag) {
		...
		if (decomp_flag == 1) {
			...
			if (Loss >= ACCEPTANCE * g_Loss[Idx]) {
				...
				memcpy(g_grad + Idx * vnum, grad, vnum * sizeof(float));
				memcpy(g_Hessian + Idx * vnum * vnum, Hessian, vnum * vnum * sizeof(float));
				g_iter_push[Idx] = (fabs(Loss - g_Loss[Idx]) >= (fabs(Loss) * OPTTOL));
				g_Loss[Idx] = Loss;
			}
		}
...
__global__ void getCRLB_7_1ch(int NFits, int boxsz, int splinesz, float* g_var, float* coeff_PSF, float* g_xvec, float* g_CRLB)
{
	const int nnum = 7, nchannels = 1, vnum0 = NDIM + nchannels;
	const int vnum = nnum * vnum0 + nchannels;
	unsigned int Idx = blockIdx.x * blockDim.x + threadIdx.x;
	...
	if (Idx < NFits) {
		...
		get_FIM_1ch(boxsz, splinesz, varim, coeff_PSF, nnum, xvec,
			loc, offset_x, offset_y, deltaf, ddeltaf_dx, ddeltaf_dy, psf, dpsf_dloc, deriv1, FIM);
...
__global__ void getCRLB_7_2ch(int NFits, int boxsz, int splinesz, float* g_var, float* coeff_PSF, 
	int warpdeg, int* g_lu, float* coeff_R2T, float* g_xvec, float* g_CRLB)
{
	...
	if (Idx < NFits) {
		...
		get_FIM_2ch(boxsz, splinesz, varim, coeff_PSF, nnum, xvec, 
			warpdeg, lu, coeff_R2T, loc_ref, trans_deriv,
			loc, offset_x, offset_y, deltaf, ddeltaf_dx, ddeltaf_dy, psf, dpsf_dloc, deriv1, FIM);

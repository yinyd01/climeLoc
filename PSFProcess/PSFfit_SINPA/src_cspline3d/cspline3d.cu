...
/*! @param[in]	NFits: 				int, number of PSF squares
    @param[in]	nchannels:			int, the number of channels, vnum = NDIM + nchannels + nchannels
	@param[in]	boxsz: 				int, size of the PSF square
	@param[in]	splineszx: 			int, size of the splines (lateral)
    @param[in]	splineszz: 			int, size of the splines (axial)
    ...
    @param[in]	h_coeff_PSF: 		(nchannels, splineszx * splineszx * splineszz * 64) flattened, float, spline coefficient for the nchannel 3d PSF
...
CDLL_EXPORT void cspline3d_1ch(int NFits, int boxsz, int splineszx, int splineszz, float* h_data, float* h_var, float* h_coeff_PSF,
    float* h_xvec, float* h_CRLB, float* h_Loss, int opt, int MaxIters)
{
    ...
    cudasafe(cudaMalloc((void**)&d_coeff_PSF, splineszx * splineszx * splineszz * 64 * sizeof(float)), "malloc d_coeff_PSF", __LINE__);
    ...
    cudasafe(cudaMemcpy(d_coeff_PSF, h_coeff_PSF, splineszx * splineszx * splineszz * 64 * sizeof(float), cudaMemcpyHostToDevice), "transfer h_coeff_PSF to device", __LINE__);
    if (h_var) {
    ...
    Init_1ch<<<grids, blcks>>>(NFits, boxsz, splineszx, splineszz, d_data, d_var, d_coeff_PSF,
        d_xvec, d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_push_flag, opt);

    for (int iter = 0; iter < MaxIters; iter++)
        LMupdate_1ch<<<grids, blcks>>>(NFits, d_push_flag, boxsz, splineszx, splineszz, d_data, d_var, d_coeff_PSF, 
            d_maxJump, d_lambda, d_xvec, d_grad, d_Hessian, d_pvec, d_Loss, opt);
    
    getCRLB_1ch<<<grids, blcks>>>(NFits, boxsz, splineszx, splineszz, d_var, d_coeff_PSF, d_xvec, d_Loss, d_Hessian, d_CRLB, opt);
    ...
    cudasafe(cudaFree(d_coeff_PSF), "free d_coeff on device", __LINE__);
...
CDLL_EXPORT void cspline3d_2ch(int NFits, int boxsz, int splineszx, int splineszz, float* h_data, float* h_var, float* h_coeff_PSF,
    ...
    float* h_xvec, float* h_CRLB, float* h_Loss, int opt, int MaxIters)
{
    ...
    Init_2ch<<<grids, blcks>>>(NFits, boxsz, splineszx, splineszz, d_data, d_var, d_coeff_PSF, warpdeg, d_lu, d_coeff_R2T, 
        d_xvec, d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_push_flag, opt);

    for (int iter = 0; iter < MaxIters; iter++)
        LMupdate_2ch<<<grids, blcks>>>(NFits, d_push_flag, boxsz, splineszx, splineszz, d_data, d_var, d_coeff_PSF, warpdeg, d_lu, d_coeff_R2T, 
            d_maxJump, d_lambda, d_xvec, d_grad, d_Hessian, d_pvec, d_Loss, opt);
    
    getCRLB_2ch<<<grids, blcks>>>(NFits, boxsz, splineszx, splineszz, d_var, d_coeff_PSF, warpdeg, d_lu, d_coeff_R2T,
...
CDLL_EXPORT void cspline3d_BP(int NFits, int boxsz, int splineszx, int splineszz, float* h_data, float* h_var, float* h_coeff_PSF,
    int warpdeg, int* h_lu, float* h_coeff_R2T,
    float* h_xvec, float* h_CRLB, float* h_Loss, int opt, int MaxIters)
{
    const int nchannels = 2;
    ...
    Init_BP<<<grids, blcks>>>(NFits, boxsz, splineszx, splineszz, d_data, d_var, d_coeff_PSF, warpdeg, d_lu, d_coeff_R2T, 
        d_xvec, d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_push_flag, opt);

    for (int iter = 0; iter < MaxIters; iter++)
        LMupdate_BP<<<grids, blcks>>>(NFits, d_push_flag, boxsz, splineszx, splineszz, d_data, d_var, d_coeff_PSF, warpdeg, d_lu, d_coeff_R2T, 
            d_maxJump, d_lambda, d_xvec, d_grad, d_Hessian, d_pvec, d_Loss, opt);
    
    getCRLB_BP<<<grids, blcks>>>(NFits, boxsz, splineszx, splineszz, d_var, d_coeff_PSF, warpdeg, d_lu, d_coeff_R2T,
        d_xvec, d_Loss, d_Hessian, d_CRLB, opt);

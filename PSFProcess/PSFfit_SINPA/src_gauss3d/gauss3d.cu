...
/*! @param[in]	NFits: 				int, number of PSF squares
    ...
    @param[in]	h_astigs:			(nchannels, 9) flattened, float, [PSFsigmax0, PSFsigmay0, shiftx, shifty, dof, Ax, Bx, Ay, By] the astigmatic parameters for each channel
...
CDLL_EXPORT void gauss3d_1ch(int NFits, int boxsz, int zrange, float* h_data, float* h_var, float* h_astigs,
    float* h_xvec, float* h_CRLB, float* h_Loss, int opt, int MaxIters)
{
    ...
    cudasafe(cudaMalloc((void**)&d_astigs, 9 * sizeof(float)), "malloc d_astigs", __LINE__);
    ...
    cudasafe(cudaMemcpy(d_astigs, h_astigs, 9 * sizeof(float), cudaMemcpyHostToDevice), "transfer h_astigs to device", __LINE__);
    if (h_var) {
    ...
    Init_1ch<<<grids, blcks>>>(NFits, boxsz, zrange, d_data, d_var, d_astigs,
        d_xvec, d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_push_flag, opt);

    for (int iter = 0; iter < MaxIters; iter++)
        LMupdate_1ch<<<grids, blcks>>>(NFits, d_push_flag, boxsz, zrange, d_data, d_var, d_astigs,
            d_maxJump, d_lambda, d_xvec, d_grad, d_Hessian, d_pvec, d_Loss, opt);
    
    getCRLB_1ch<<<grids, blcks>>>(NFits, boxsz, d_var, d_astigs, d_xvec, d_Loss, d_Hessian, d_CRLB, opt);
...
CDLL_EXPORT void gauss3d_2ch(int NFits, int boxsz, int zrange, float* h_data, float* h_var, float* h_astigs,
    ...
    float* h_xvec, float* h_CRLB, float* h_Loss, int opt, int MaxIters)
{
    ...
    Init_2ch<<<grids, blcks>>>(NFits, boxsz, zrange, d_data, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T,
        d_xvec, d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_push_flag, opt);

    for (int iter = 0; iter < MaxIters; iter++)
        LMupdate_2ch<<<grids, blcks>>>(NFits, d_push_flag, boxsz, zrange, d_data, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T,
            d_maxJump, d_lambda, d_xvec, d_grad, d_Hessian, d_pvec, d_Loss, opt);
    
    getCRLB_2ch<<<grids, blcks>>>(NFits, boxsz, d_var, d_astigs, warpdeg, d_lu, d_coeff_R2T,

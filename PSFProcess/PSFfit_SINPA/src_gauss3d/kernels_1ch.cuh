...
*/
__global__ void Init_1ch(int NFits, int boxsz, int zrange, float* g_data, float* g_var, float* astigs,
	float* g_xvec, float* g_maxJump, float* g_Loss, float* g_grad, float* g_Hessian, float* g_pvec, float* g_lambda, char* g_push_flag, int opt)
{
	...
	if (Idx < NFits) {
		...
		init_maxJump_1ch(boxsz, zrange, xvec, maxJump);
		...
		for (pxID = 0, Loss = 0.0f; pxID < boxsz * boxsz; pxID++) {
			data = dataim[pxID];
			fconstruct_gauss3D(pxID, boxsz, astigs, xvec, &psf, dpsf_dloc);
...
*/
__global__ void LMupdate_1ch(int NFits, char* g_push_flag, int boxsz, int zrange, float* g_data, float* g_var, float* astigs, 
	float* g_maxJump, float* g_lambda, float* g_xvec, float* g_grad, float* g_Hessian, float* g_pvec, float* g_Loss, int opt)
{
	...
	if (push_flag) {
		...
		if ( decomp_flag == 1 ) {
			...
			boxConstrain_1ch(boxsz, zrange, xvec);
...
/*!	@brief	<<<ceil(NFits/BLCKSZ), BLCKSZ = 64>>>
			calculate the CRLB	
*/
__global__ void getCRLB_1ch(int NFits, int boxsz, float* g_var, float* astigs, 

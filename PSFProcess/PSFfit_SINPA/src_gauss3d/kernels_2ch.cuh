...
*/
__global__ void Init_2ch(int NFits, int boxsz, int zrange, float* g_data, float* g_var, float* g_astigs,
	...
	float* g_xvec, float* g_maxJump, float* g_Loss, float* g_grad, float* g_Hessian, float* g_pvec, float* g_lambda, char* g_push_flag, int opt)
{
	...
	float *astigs = nullptr, psf = 0.0f, loc[NDIM] = {0.0f}, dpsf_dloc[3] = {0.0f};
	...
	if (Idx < NFits) {
		...
		init_maxJump_2ch(boxsz, zrange, xvec, maxJump);
		...
		for (chID = 0, Loss = 0.0f; chID < nchannels; chID++) {
			...
			for (pxID = 0; pxID < boxsz * boxsz; pxID++) {
				data = dataim[pxID];
				fconstruct_gauss3D(pxID, boxsz, astigs, loc, &psf, dpsf_dloc);
...
*/
__global__ void LMupdate_2ch(int NFits, char* g_push_flag, int boxsz, int zrange, float* g_data, float* g_var, float* g_astigs, 
	int warpdeg, int* g_lu, float* g_coeff_R2T, 
	float* g_maxJump, float* g_lambda, float* g_xvec, float* g_grad, float* g_Hessian, float* g_pvec, float* g_Loss, int opt)
{
	...
	float *astigs = nullptr, psf = 0.0f, loc[NDIM] = {0.0f}, dpsf_dloc[NDIM] = {0.0f};
	float data = 0.0f, model = 0.0f, Loss = 0.0f;
	float deriv1[VNUM_2CH] = {0.0f}, grad[VNUM_2CH] = {0.0f}, Hessian[VNUM_2CH * VNUM_2CH] = {0.0f}; 
	...
	if (push_flag) {
		...
		if ( decomp_flag == 1 ) {
			...
			boxConstrain_2ch(boxsz, zrange, xvec);
...
/*!	@brief	<<<ceil(NFits/BLCKSZ), BLCKSZ = 64>>>
			calculate the CRLB	
*/
__global__ void getCRLB_2ch(int NFits, int boxsz, float* g_var, float* g_astigs, 
	int warpdeg, int* g_lu, float* g_coeff_R2T, 

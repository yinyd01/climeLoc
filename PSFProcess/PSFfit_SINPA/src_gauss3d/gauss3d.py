...
def gauss3d_1ch(data, var, zrange, astigs, zcoarse, optmethod, iterations, batsz):
    ...
    for i in range(nbats):
        ...
        if var is None:
            fitter(dum_NFits, boxsz, zrange, dum_data, None, astigs, zcoarse, dum_xvec, dum_crlb, dum_loss, opt, iterations)
        else:
            dum_var = var[i*batsz : i*batsz + dum_NFits]
            dum_var = np.ascontiguousarray(dum_var.flatten(), dtype=np.float32)
            fitter(dum_NFits, boxsz, zrange, dum_data, dum_var, astigs, zcoarse, dum_xvec, dum_crlb, dum_loss, opt, iterations)
...
def gauss3d_2ch(linkN, data, var, zrange, astigs, zcoarse, lc, uc, coeff_R2T, fracN, optmethod, iterations, batsz):
    ...
    if len(astigs) != nchannels:
        raise ValueError("nchannel mismatch: astigs.nchannels={}, data.nchannels={}".format(len(astigs), nchannels))
        ...
        if linkN == 1:
            if var is None:
                for j in range(nfluorophores):
                    fitter(linkN, dum_NFits, boxsz, zrange, dum_data, None, astigs, zcoarse, warpdeg, dum_lu, coeff_R2T, fracN[j],
                ...
                dum_var = np.ascontiguousarray(dum_var.flatten(), dtype=np.float32)
                for j in range(nfluorophores):
                    fitter(linkN, dum_NFits, boxsz, zrange, dum_data, dum_var, astigs, zcoarse, warpdeg, dum_lu, coeff_R2T, fracN[j],
                            dum_xvec, dum_crlb, dum_loss, opt, iterations)
                    raw_xvec[i*batsz : i*batsz + dum_NFits, j] = dum_xvec.reshape((dum_NFits, vnum))
                    raw_crlb[i*batsz : i*batsz + dum_NFits, j] = dum_crlb.reshape((dum_NFits, vnum))
                    raw_Loss[i*batsz : i*batsz + dum_NFits, j] = dum_loss
            ...
            if var is None:
                fitter(linkN, dum_NFits, boxsz, zrange, dum_data, None, astigs, zcoarse, warpdeg, dum_lu, coeff_R2T, 0.5,
                        dum_xvec, dum_crlb, dum_loss, opt, iterations)
            else:
                dum_var = var[i*batsz : i*batsz + dum_NFits]
                dum_var = np.ascontiguousarray(dum_var.flatten(), dtype=np.float32)
                fitter(linkN, dum_NFits, boxsz, zrange, dum_data, dum_var, astigs, zcoarse, warpdeg, dum_lu, coeff_R2T, 0.5,
                        dum_xvec, dum_crlb, dum_loss, opt, iterations)   
            xvec[i*batsz : i*batsz + dum_NFits] = dum_xvec.reshape((dum_NFits, vnum))
            crlb[i*batsz : i*batsz + dum_NFits] = dum_crlb.reshape((dum_NFits, vnum))
            Loss[i*batsz : i*batsz + dum_NFits] = dum_loss

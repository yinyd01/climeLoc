...
#ifndef KERNELS_LINK0_CUH
#define KERNELS_LINK0_CUH



/*! @brief	kernel functions for dualview PSFfit_SINPA
	@param		nchannels:			const int, 2, the number of channels 
	@param		vnum:				const int, NDIM + 4, the number of optimizatin parameters
...
/*! @brief	<<<ceil(NFits/BLCKSZ), BLCKSZ = 64>>>
			...
			Initialize the gradient and Hessian.
...
*/
__global__ void Init_2ch(int NFits, int boxsz, int splinesz, float* g_data, float* g_var, float* g_coeff_PSF, 
	...
	float* g_xvec, float* g_maxJump, float* g_Loss, float* g_grad, float* g_Hessian, float* g_pvec, float* g_lambda, char* g_push_flag, int opt)
{
	...
	float *coeff_PSF = nullptr, deltaf[16] = {0.0f}, ddeltaf_dx[16] = {0.0f}, ddeltaf_dy[16] = {0.0f};
	...
	if (Idx < NFits) {
		...
		for (chID = 0, Loss = 0.0f; chID < nchannels; chID++) {
			...
			coeff_PSF = g_coeff_PSF + chID * splinesz * splinesz * 16;
			if (chID == 0) {
			...
			DeltaConstruct2D(splinesz, loc, &offset_x, &offset_y, deltaf, ddeltaf_dx, ddeltaf_dy);
...
*/
__global__ void LMupdate_2ch(int NFits, char* g_push_flag, int boxsz, int splinesz, float* g_data, float* g_var, float* g_coeff_PSF, 
	int warpdeg, int* g_lu, float* g_coeff_R2T,
...
/*!	@brief	<<<ceil(NFits/BLCKSZ), BLCKSZ = 64>>>
			calculate the CRLB	
*/
__global__ void getCRLB_2ch(int NFits, int boxsz, int splinesz, float* g_var, float* g_coeff_PSF, 
	int warpdeg, int* g_lu, float* g_coeff_R2T, 

...
/*! @param[in]	NFits: 				int, number of PSF squares
    ...
    @param		d_push_flag: 		(NFits) char, flag for whether iteration should proceed.
...
CDLL_EXPORT void cspline2d_1ch(int NFits, int boxsz, int splinesz, float* h_data, float* h_var, float* h_coeff_PSF, 
    float* h_xvec, float* h_CRLB, float* h_Loss, int opt, int MaxIters)
{
    ...
    cudasafe(cudaMalloc((void**)&d_coeff_PSF, splinesz * splinesz * 16 * sizeof(float)), "malloc d_coeff_PSF", __LINE__);
    ...
    cudasafe(cudaMemcpy(d_coeff_PSF, h_coeff_PSF, splinesz * splinesz * 16 * sizeof(float), cudaMemcpyHostToDevice), "transfer h_coeff_PSF to device", __LINE__);
    if (h_var) {
    ...
    Init_1ch<<<grids, blcks>>>(NFits, boxsz, splinesz, d_data, d_var, d_coeff_PSF, 
        d_xvec, d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_push_flag, opt);

    for (int iter = 0; iter < MaxIters; iter++)
        LMupdate_1ch<<<grids, blcks>>>(NFits, d_push_flag, boxsz, splinesz, d_data, d_var, d_coeff_PSF, 
            d_maxJump, d_lambda, d_xvec, d_grad, d_Hessian, d_pvec, d_Loss, opt);
    
    getCRLB_1ch<<<grids, blcks>>>(NFits, boxsz, splinesz, d_var, d_coeff_PSF, 
...
CDLL_EXPORT void cspline2d_2ch(int NFits, int boxsz, int splinesz, float* h_data, float* h_var, float* h_coeff_PSF, 
    ...
    float* h_xvec, float* h_CRLB, float* h_Loss, int opt, int MaxIters)
{
    ...
    Init_2ch<<<grids, blcks>>>(NFits, boxsz, splinesz, d_data, d_var, d_coeff_PSF, warpdeg, d_lu, d_coeff_R2T,
        d_xvec, d_maxJump, d_Loss, d_grad, d_Hessian, d_pvec, d_lambda, d_push_flag, opt);

    for (int iter = 0; iter < MaxIters; iter++)
        LMupdate_2ch<<<grids, blcks>>>(NFits, d_push_flag, boxsz, splinesz, d_data, d_var, d_coeff_PSF, warpdeg, d_lu, d_coeff_R2T,
            d_maxJump, d_lambda, d_xvec, d_grad, d_Hessian, d_pvec, d_Loss, opt);
    
    getCRLB_2ch<<<grids, blcks>>>(NFits, boxsz, splinesz, d_var, d_coeff_PSF, warpdeg, d_lu, d_coeff_R2T,
        d_xvec, d_Loss, d_Hessian, d_CRLB, opt);

...
def cspline2d_1ch(data, var, coeff_PSF, optmethod, iterations, batsz):
...
def cspline2d_2ch(linkN, data, var, coeff_PSF, lc, uc, coeff_R2T, fracN, optmethod, iterations, batsz):
        ...
        if linkN == 1:
            if var is None:
                for j in range(nfluorophores):
                    fitter(1, dum_NFits, boxsz, splinesz, dum_data, None, coeff_PSF, warpdeg, dum_lu, coeff_R2T, fracN[j],
                ...
                dum_var = np.ascontiguousarray(dum_var.flatten(), dtype=np.float32)
                for j in range(nfluorophores):
                    fitter(1, dum_NFits, boxsz, splinesz, dum_data, dum_var, coeff_PSF, warpdeg, dum_lu, coeff_R2T, fracN[j],
                            dum_xvec, dum_crlb, dum_loss, opt, iterations)
                    raw_xvec[i*batsz : i*batsz + dum_NFits, j] = dum_xvec.reshape((dum_NFits, vnum))
                    raw_crlb[i*batsz : i*batsz + dum_NFits, j] = dum_crlb.reshape((dum_NFits, vnum))
                    raw_Loss[i*batsz : i*batsz + dum_NFits, j] = dum_loss
            ...
            if var is None:
                fitter(0, dum_NFits, boxsz, splinesz, dum_data, None, coeff_PSF, warpdeg, dum_lu, coeff_R2T, 0.5,
                        dum_xvec, dum_crlb, dum_loss, opt, iterations)
            else:
                dum_var = var[i*batsz : i*batsz + dum_NFits]
                dum_var = np.ascontiguousarray(dum_var.flatten(), dtype=np.float32)
                fitter(0, dum_NFits, boxsz, splinesz, dum_data, dum_var, coeff_PSF, warpdeg, dum_lu, coeff_R2T, 0.5,
                        dum_xvec, dum_crlb, dum_loss, opt, iterations)
            xvec[i*batsz : i*batsz + dum_NFits] = dum_xvec.reshape((dum_NFits, vnum))
            crlb[i*batsz : i*batsz + dum_NFits] = dum_crlb.reshape((dum_NFits, vnum))
            Loss[i*batsz : i*batsz + dum_NFits] = dum_loss

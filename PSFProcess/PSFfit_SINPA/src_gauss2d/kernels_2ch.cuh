...
#ifndef KERNELS_2CH_CUH
#define KERNELS_2CH_CUH



/*! @brief	kernel functions for dual-channel PSFfit_SINPA
	@param		nchannels:			const int, 2, the number of channels 
	@param		vnum:				const int, NDIM + 4 - linkN, the number of optimizatin parameters
	@param[in]	NFits: 				int, number of PSF squares
	@param[in]	boxsz: 				int, size of the PSF square
	@param[in]	g_data: 			(NFits * nchannels * boxsz * boxsz) float, single-channel PSF data
	@param[in]	g_var: 				(NFits * nchannels * boxsz * boxsz) float, single-channel pixel-dependent variance of camera readout noise
	@param[in]	PSFsigmax: 			(nchannels) float, sigma (x-axis) of the Gaussian PSF
	@param[in]	PSFsigmay: 			(nchannels) float, sigma (y-axis) of the Gaussian PSF
...
*/
__global__ void Init_2ch(int NFits, int boxsz, float* g_data, float* g_var, float* PSFsigmax, float* PSFsigmay, 
	...
	float* g_xvec, float* g_maxJump, float* g_Loss, float* g_grad, float* g_Hessian, float* g_pvec, float* g_lambda, char* g_push_flag, int opt)
{
	...
	float deriv1[VNUM_2CH] = {0.0f}, grad[VNUM_2CH] = {0.0f}, Hessian[VNUM_2CH * VNUM_2CH] = {0.0f};
	...
	if (Idx < NFits) {
		...
		init_maxJump_2ch(boxsz, xvec, maxJump);	
		...
		for (chID = 0, Loss = 0.0f; chID < nchannels; chID++) {
			...
			for (pxID = 0; pxID < boxsz * boxsz; pxID++) {
				data = dataim[pxID];
				fconstruct_gauss2D(pxID, boxsz, PSFsigmax[chID], PSFsigmay[chID], loc, &psf, dpsf_dloc);
...
*/
__global__ void LMupdate_2ch(int NFits, char* g_push_flag, int boxsz, float* g_data, float* g_var, float* PSFsigmax, float* PSFsigmay, 
	int warpdeg, int* g_lu, float* g_coeff_R2T,
	float* g_maxJump, float* g_lambda, float* g_xvec, float* g_grad, float* g_Hessian, float* g_pvec, float* g_Loss, int opt)
{
	...
	float xvec[VNUM_2CH] = {0.0f}, pvec[VNUM_2CH] = {0.0f}, maxJump[VNUM_2CH] = {0.0f}, L[VNUM_2CH * VNUM_2CH] = {0.0f};
...
/*!	@brief	<<<ceil(NFits/BLCKSZ), BLCKSZ = 64>>>
			calculate the CRLB	
*/
__global__ void getCRLB_2ch(int NFits, int boxsz, float* g_var, float* PSFsigmax, float* PSFsigmay, 
	int warpdeg, int* g_lu, float* g_coeff_R2T, 
	float* g_xvec, float* g_Loss, float* g_Hessian, float* g_CRLB, int opt)
{
	...
	float psf = 0.0f, loc[NDIM] = {0.0f}, dpsf_dloc[NDIM] = {0.0f};
	float model = 0.0f, deriv1[VNUM_2CH] = {0.0f}, FIM[VNUM_2CH * VNUM_2CH] = {0.0f};
	float xvec[VNUM_2CH] = {0.0f}, L[VNUM_2CH * VNUM_2CH] = {0.0f}; 

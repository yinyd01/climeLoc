...
*/
__global__ void Init_1ch(int NFits, int boxsz, float* g_data, float* g_var, float* PSFsigmax, float* PSFsigmay, 
	float* g_xvec, float* g_maxJump, float* g_Loss, float* g_grad, float* g_Hessian, float* g_pvec, float* g_lambda, char* g_push_flag, int opt)
{
	...
	if (Idx < NFits) {
		...
		for (pxID = 0, Loss = 0.0f; pxID < boxsz * boxsz; pxID++) {
			data = dataim[pxID];
			fconstruct_gauss2D(pxID, boxsz, PSFsigmax[0], PSFsigmay[0], xvec, &psf, dpsf_dloc);
...
*/
__global__ void LMupdate_1ch(int NFits, char* g_push_flag, int boxsz, float* g_data, float* g_var, float* PSFsigmax, float* PSFsigmay, 
...
/*!	@brief	<<<ceil(NFits/BLCKSZ), BLCKSZ = 64>>>
			calculate the CRLB	
*/
__global__ void getCRLB_1ch(int NFits, int boxsz, float* g_var, float* PSFsigmax, float* PSFsigmay, 

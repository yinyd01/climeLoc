...
def _gauss2d_1ch(data, var, PSFsigmax, PSFsigmay, optmethod, iterations, batsz):
...
def _gauss2d_2ch(data, var, PSFsigmax, PSFsigmay, lc, uc, coeff_R2T, optmethod, iterations, batsz):
    ...
    for i in range(nbats):
        ...
        if var is None:
            fitter(dum_NFits, boxsz, dum_data, None, PSFsigmax, PSFsigmay, warpdeg, dum_lu, coeff_R2T,
                    dum_xvec, dum_crlb, dum_loss, opt, iterations)
        else:
            dum_var = var[i*batsz : i*batsz + dum_NFits]
            dum_var = np.ascontiguousarray(dum_var.flatten(), dtype=np.float32)
            fitter(dum_NFits, boxsz, dum_data, dum_var, PSFsigmax, PSFsigmay, warpdeg, dum_lu, coeff_R2T,
                    dum_xvec, dum_crlb, dum_loss, opt, iterations) 
        xvec[i*batsz : i*batsz + dum_NFits] = dum_xvec.reshape((dum_NFits, vnum))
        crlb[i*batsz : i*batsz + dum_NFits] = dum_crlb.reshape((dum_NFits, vnum))
        Loss[i*batsz : i*batsz + dum_NFits] = dum_loss
...
def _gauss3d_1ch(data, var, zrange, astigs, optmethod, iterations, batsz):
...
def _gauss3d_2ch(data, var, zrange, astigs, lc, uc, coeff_R2T, optmethod, iterations, batsz):
    ...
    assert np.any(uc.shape == (nchannels, NFits)), "shape mismatch: uc.(nchannels, NFits)={}, data.(nchannels, NFits)={}".format(uc.shape, (nchannels, NFits))
    lu = np.transpose(np.array([lc, uc]), (2, 1, 0))
    assert len(astigs) == nchannels, "nchannel mismatch: astigs.nchannels={}, data.nchannels={}".format(len(astigs), nchannels)
    assert len(coeff_R2T) == nchannels - 1, "nchannel mismatch: coeff_R2T.nchannels={}, data.nchannels={}".format(len(coeff_R2T), nchannels)
    warpdeg = _perfect_sqrt(len(coeff_R2T[0][0]))
    ...
    for i in range(nbats):
        ...
        if var is None:
            fitter(dum_NFits, boxsz, zrange, dum_data, None, astigs, warpdeg, dum_lu, coeff_R2T,
                    dum_xvec, dum_crlb, dum_loss, opt, iterations)
        else:
            dum_var = var[i*batsz : i*batsz + dum_NFits]
            dum_var = np.ascontiguousarray(dum_var.flatten(), dtype=np.float32)
            fitter(dum_NFits, boxsz, zrange, dum_data, dum_var, astigs, warpdeg, dum_lu, coeff_R2T,
                    dum_xvec, dum_crlb, dum_loss, opt, iterations)   
        xvec[i*batsz : i*batsz + dum_NFits] = dum_xvec.reshape((dum_NFits, vnum))
        crlb[i*batsz : i*batsz + dum_NFits] = dum_crlb.reshape((dum_NFits, vnum))
        Loss[i*batsz : i*batsz + dum_NFits] = dum_loss
...
"""
def sinpa_gauss_1ch(data, var, PSF, optmethod='MLE', iterations=100, batsz=65535):
    
    modality = PSF['modality']
    if modality == '2Dfrm':
        PSFsigmax = PSF['PSFsigmax'][:1]
        PSFsigmay = PSF['PSFsigmay'][:1]
        xvec, crlb, Loss = _gauss2d_1ch(data, var, PSFsigmax, PSFsigmay, optmethod, iterations, batsz)
        ...
        xvec, crlb, Loss = _gauss3d_1ch(data, var, zrange, astigs, optmethod, iterations, batsz)
        xvec[:, 0] -= spl_wobblex(xvec[:, 2])
        xvec[:, 1] -= spl_wobbley(xvec[:, 2])
        
    else:
        raise ValueError("only '2Dfrm' or 'AS' is available for sinpa_gauss_1ch")
    return xvec, crlb, Loss



def sinpa_gauss_2ch(data, varim, PSF, lc, uc, coeff_R2T, optmethod='MLE', iterations=100, batsz=65535):
    
    modality = PSF['modality']
    if modality == '2Dfrm':
        if 'zrange' in PSF.keys():
            raise ValueError("modality mismatch: input.modality='2D', PSF.modality='3D'")
        PSFsigmax = PSF['PSFsigmax']
        PSFsigmay = PSF['PSFsigmay']
        xvec, crlb, Loss = _gauss2d_2ch(data, varim, PSFsigmax, PSFsigmay, lc, uc, coeff_R2T, optmethod, iterations, batsz)
        ...
        spl_wobblex = PSF['spl_wobblex'][0]
        spl_wobbley = PSF['spl_wobbley'][0]
        xvec, crlb, Loss = _gauss3d_2ch(data, varim, zrange, astigs, lc, uc, coeff_R2T, optmethod, iterations, batsz)
        xvec[:, 0] -= spl_wobblex(xvec[:, 2])
        xvec[:, 1] -= spl_wobbley(xvec[:, 2])

    else:
        raise ValueError("only '2Dfrm' or 'AS' is available for the modality")
    return xvec, crlb, Loss
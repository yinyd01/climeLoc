...
def dbscan(locs, epsilon, minpts):
    ...
    locs = np.ascontiguousarray(np.array(locs).flatten(), dtype=np.float32)
    epsilon = np.ascontiguousarray(np.array(epsilon).flatten(), dtype=np.float32)
...
if __name__ == '__main__':
    ...
    # simulations
    centers = [[1, 1], [-1, -1], [1, -1]]
    X, labels_true = make_blobs(n_samples=131072, centers=centers, cluster_std=0.2, random_state=0)
    ...
    # sklearn dbscan
    tas = time.time() 
    db = DBSCAN(eps=epsilon[0], min_samples=minpts).fit(X)
    labels = db.labels_
    core_samples_mask = np.zeros_like(labels, dtype=bool)
    core_samples_mask[db.core_sample_indices_] = True
    print(time.time()-tas, set(labels))
    ...
    # cpp
    tas = time.time()
    classification, iscore = dbscan(X, epsilon, minpts)
    print(time.time()-tas, set(classification))
    n_clusters_cpp = len(set(classification)) - (1 if -1 in labels else 0)
    n_noise_cpp = list(classification).count(-1)
    n_unclass_cpp = list(classification).count(0)
